...
USE [ENBD_MISDB]
GO
/****** Object:  StoredProcedure [PRO].[UpdateNetBalance_AccountWise]    Script Date: 5/8/2025 11:33:21 AM ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [PRO].[UpdateNetBalance_AccountWise]       
...
SET NetBalance = ISNULL(PrincOutStd,0)
...
UPDATE A    SET NetBalance =  ISNULL(PrincOutStd,0)-isnull(Amount,0)
        FROM PRO.AccountCal A      
        INNER JOIN DimAssetClass B ON B.AssetClassAlt_Key=isnull(A.FinalAssetClassAlt_Key,1)
                        AND (B.EffectiveFromTimeKey<=@TimeKey AND B.EffectiveToTimeKey>=@TimeKey)
