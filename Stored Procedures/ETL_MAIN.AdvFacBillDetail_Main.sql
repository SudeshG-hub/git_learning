...
----Select * 
from ENBD_TEMPDB.DBO.TempAdvFacBillDetail A
...
INNER JOIN ENBD_TEMPDB.DBO.TempAdvFacBillDetail AS T
...
AND NOT EXISTS (SELECT 1 FROM ENBD_TEMPDB.DBO.TempAdvFacBillDetail BB
...
SELECT @EntityKey=MAX(EntityKey) FROM  ENBD_MISDB.[dbo].[AdvFacBillDetail] 
...
SET TEMP.EntityKey=ACCT.EntityKey
 FROM ENBD_TEMPDB.DBO.[TempAdvFacBillDetail] TEMP
INNER JOIN (SELECT AccountEntityid,(@EntityKey + ROW_NUMBER()OVER(ORDER BY (SELECT 1))) EntityKey
			FROM ENBD_TEMPDB.DBO.[TempAdvFacBillDetail]
...
SELECT
      ...
      ,[ScrCrErrorSeq]
      ,[ConsigmentExport]
		   FROM ENBD_TEMPDB.dbo.TempAdvFacBillDetail T Where ISNULL(T.IsChanged,'U') IN ('N','C')
