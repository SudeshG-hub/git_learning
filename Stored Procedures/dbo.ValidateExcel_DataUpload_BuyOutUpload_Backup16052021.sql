...
CREATE PROCEDURE [dbo].[ValidateExcel_DataUpload_BuyOutUpload_Backup16052021] 
@MenuID INT=1466,  
...
END
  ...
  --select * from BuyoutDetails_stg
	...
	SET  
		...
		InterestReceivable,Charges,AccuredInterest,DPD,AssetClass,Action'    
		,Srnooferroneousrows=''
 FROM UploadBuyout V  
 WHERE ISNULL(SlNo,'')=''
AND ISNULL(AUNo,'')=''
...
WHERE len(PAN)<>10
  ...
  /* UPDATE UploadBuyout
	SET  
        ErrorMessage=CASE WHEN ISNULL(ErrorMessage,'')='' THEN 'AadharNo is not valid.  Please check the values and upload again'     
					ELSE ErrorMessage+','+SPACE(1)+'AadharNo is not valid.  Please check the values and upload again'     END
...
FROM UploadBuyout V  
 WHERE len(AadharNo)<>10 */

 
  UPDATE UploadBuyout
	SET  
        ErrorMessage=CASE WHEN ISNULL(ErrorMessage,'')='' THEN 'Pan is not valid.  Please check the values and upload again'     
					ELSE ErrorMessage+','+SPACE(1)+'pan is not valid.  Please check the values and upload again'     END
		,ErrorinColumn=CASE WHEN ISNULL(ErrorinColumn,'')='' THEN 'pan' ELSE ErrorinColumn +','+SPACE(1)+  'pan' END  
		,Srnooferroneousrows=V.SlNo
--								----STUFF((SELECT ','+SRNO 
--								----FROM UploadBuyout A
