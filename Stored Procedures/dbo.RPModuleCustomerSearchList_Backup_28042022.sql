...
CREATE PROC [dbo].[RPModuleCustomerSearchList_Backup_28042022]
--Declare
													
													--@PageNo         INT         = 1, 
													--@PageSize       INT         = 10, 
													@OperationFlag  INT         = 2
													,@CustomerID Varchar(20)='22130328'
													,@MenuID  INT  =14569
AS
 ...
 A.EffectiveToTimeKey, 
				...
				IF OBJECT_ID('temp31') IS NOT NULL  
				 ...
				 SET ReferenceDate=Case  When ExposureBucketAlt_Key=1  Then Convert(Date,ISNULL(BorrowerDefaultDate,'1900-01-01'))
				        When ExposureBucketAlt_Key=3 AND Convert(Date,'2019-06-07' )<Convert(Date,ISNULL(BorrowerDefaultDate,'1900-01-01')) Then Convert(Date,ISNULL(BorrowerDefaultDate,'1900-01-01'))
				 ...
				 SET ReferenceDate=Case  When ExposureBucketAlt_Key=1  
				     ...
				     When ExposureBucketAlt_Key=3 AND Convert(Date,ISNULL(ReferenceDate,'1900-01-01') )>=Convert(Date,ISNULL
										                  (BorrowerDefaultDate,'1900-01-01')) 
														  
										 Then Convert (Date,ISNULL(ReferenceDate,'1900-01-01'))
						                 When ExposureBucketAlt_Key=3 AND Convert(Date,ISNULL(ReferenceDate,'1900-01-01') )<Convert(Date,ISNULL                                                                                   (BorrowerDefaultDate,'1900-01-01')) 
										 Then   Convert(Date,BorrowerDefaultDate )

						                 When ExposureBucketAlt_Key=2 AND Convert(Date,ISNULL(ReferenceDate,'1900-01-01') )>=Convert(Date,ISNULL                                                                         (BorrowerDefaultDate,'1900-01-01')) 
										 Then Convert(Date,ISNULL(ReferenceDate,'1900-01-01'))  
						  When ExposureBucketAlt_Key=2 AND Convert(Date,ISNULL(ReferenceDate,'1900-01-01') )<Convert(Date,ISNULL(BorrowerDefaultDate,'1900-01-01')) Then   Convert(Date,BorrowerDefaultDate ) END
