...
----Select * 
from ENBD_TEMPDB.DBO.TempAdvFacDLDetail A
...
INNER JOIN ENBD_TEMPDB.DBO.TempAdvFacDLDetail AS T
...
AND NOT EXISTS (SELECT 1 FROM ENBD_TEMPDB.DBO.TempAdvFacDLDetail BB
...
SELECT @EntityKey=MAX(EntityKey) FROM  ENBD_MISDB.[dbo].[AdvFacDLDetail] 
...
SELECT
      ...
      ,[ScrCrErrorSeq]
      ,[CoverExpiryDt]
		   FROM ENBD_TEMPDB.dbo.TempAdvFacDLDetail T Where ISNULL(T.IsChanged,'U') IN ('N','C')
