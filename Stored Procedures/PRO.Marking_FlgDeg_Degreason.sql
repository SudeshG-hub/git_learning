...
/* LOAN BUYOUT DPD CALCULATION */
	...
	UPDATE A
		SET  A.FLGDEG='Y'
			,DegReason =CASE --------WHEN  PeakDPD>=A.REFPERIODOVERDUE THEN 'NPA due to Months Peak DPD'
							WHEN  ISNULL(B.DPD_Seller,0)>=A.REFPERIODOVERDUE THEN 'NPA Due to overdue – Buyout Portfolio'
						END
	FROM PRO.ACCOUNTCAL A
	INNER JOIN PRO.BuyoutUploadDetailsCal B
		ON A.CustomerAcID=B.CustomerAcID
		WHERE ISNULL(A.DPD_OVERDUE,0)>=A.REFPERIODOVERDUE 
				AND A.FinalAssetClassAlt_Key=1
	/*
	UPDATE A
		SET  A.FLGDEG='Y'
			,DegReason ='NPA with Seller'
	...
	UPDATE B SET B.NPA_FLAG=CASE WHEN  ISNULL(A.DPD_OVERDUE,0)>=A.REFPERIODOVERDUE  
									THEN 'NPA_VDPD'
								 WHEN B.NPA_ClassSeller='Y' 
									THEN 'NPA_SELLER'
								ELSE B.NPA_FLAG 
							END
	FROM PRO.ACCOUNTCAL A
