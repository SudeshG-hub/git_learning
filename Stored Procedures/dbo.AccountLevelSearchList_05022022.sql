...
CREATE PROC [dbo].[AccountLevelSearchList_05022022]
...
Update A
SET A.MOCReason=ISNULL(B.ParameterAlt_Key,0),A.MOCSource = ISNULL(C.ParameterAlt_Key,0)
...
(select ParameterAlt_Key ,
			 ...
			 EffectiveFromTimeKey<=@Timekey And EffectiveToTimeKey>=@Timekey and
			  ...
			  Left JOIN 
(
			select	MOCTypeAlt_Key as ParameterAlt_Key
			 ,MOCTypeName as ParameterName
			 ,'MOCSource' as TableName
			 from dimmoctype
			 where EffectiveFromTimeKey<=@Timekey And EffectiveToTimeKey>=@Timekey

		) C
			  ON A.MOCSource_1 =C.ParameterName
