...
CREATE Procedure [dbo].[Duplicate_Verification_31102023]
	@UserId varchar(20) =''
	,@TimeKey INT = 49999
 ...
 --   Select @TimeKey=Case when ISNULL(@TimeKey,0)=0 THEN TimeKey else @TimeKey end
	...
	BEGIN
	 --select * from DimUserInfo
		if EXISTS(SELECT  1 FROM DimUserInfo WHERE  (EffectiveFromTimeKey<=@TimeKey AND EffectiveToTimeKey>=@TimeKey) 										
										AND MobileNo LIKE'%'+@Value+'%' 
										AND UserLoginID<>isnull(@UserId,'')
										AND MobileNo = @Value 
										AND UserLoginID<>isnull(@UserId,'')
										AND ISNULL(MobileNo,'')<>''
	...
	BEGIN
		if EXISTS(SELECT  1 FROM DimUserInfo WHERE 
										...
										and SUBSTRING(ISNULL(MobileNo,''),12,LEN(ISNULL(MobileNo,'')))=@Value
										--AND MobileNo = @Value AND UserLoginID<>isnull(@UserId,'')
										AND ISNULL( AuthorisationStatus,'A' ) ='A'
					UNION
					SELECT  1 FROM DimUserInfo_Mod WHERE 
