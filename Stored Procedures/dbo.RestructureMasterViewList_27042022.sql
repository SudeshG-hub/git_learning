...
-- =============================================
CREATE PROCEDURE [dbo].[RestructureMasterViewList_27042022] 
--declare
	--@OperationFlag  INT         = 1
   --,
...
IF  EXISTS(SELECT 1 FROM [CurDat].[AdvAcRestructureDetail] A WHERE (A.EffectiveFromTimeKey<=@TimeKey AND A.EffectiveToTimeKey>=@TimeKey) 
												AND A.RefSystemAcId=@RefSystemAcId AND ISNULL(A.AuthorisationStatus, 'A') = 'A')
BEGIN
IF OBJECT_ID('TempDB..#temp') IS NOT NULL
                ...
                SELECT		A.AccountEntityId
                 ...
                 (
					 ...
					 INNER JOIN  [CurDat].[AdvCustNPADetail] Y  ON Y.RefCustomerID= A.RefCustomerId
...
IF  EXISTS(SELECT 1 FROM [DBO].[AdvAcRestructureDetail_Mod] WHERE (EffectiveFromTimeKey<=@TimeKey AND EffectiveToTimeKey>=@TimeKey) 
...
IF OBJECT_ID('TempDB..#temp2') IS NOT NULL
                 ...
                 (      
					 ...
					 LEFT JOIN [PRO].[AdvAcRestructureCal] O ON O.AccountEntityId=B.AccountEntityId
					 AND O.EffectiveFromTimeKey<=@TimeKey and O.EffectiveToTimeKey>=@TimeKey
					 INNER JOIN  [dbo].[AdvCustNPADetail] Y  ON Y.RefCustomerID= D.CustomerId
					 AND Y.EffectiveFromTimeKey<=@TimeKey and Y.EffectiveToTimeKey>=@TimeKey
					 LEFT JOIN  [dbo].[DimProduct] G ON G.ProductAlt_Key = B.ProductAlt_Key
					 AND G.EffectiveFromTimeKey<=@TimeKey and G.EffectiveToTimeKey>=@TimeKey
					 LEFT JOIN  [dbo].[DimCurrency] H ON H.CurrencyAlt_Key = B.CurrencyAlt_Key
