...
BEGIN
	...
	SET NOCOUNT ON;
	DECLARE @VEFFECTIVETO INT SET @VEFFECTIVETO=(SELECT TIMEKEY-1 FROM RBL_MISDB.DBO.AUTOMATE_ADVANCES WHERE EXT_FLG='Y')


    
 ----------For New Records
UPDATE A SET A.IsChanged='N'
FROM RBL_TEMPDB.DBO.TempAdvCustRelationship A
...
INNER JOIN RBL_TEMPDB.DBO.TempAdvCustRelationship AS T
...
AND NOT EXISTS (SELECT 1 FROM RBL_TEMPDB.DBO.TempAdvCustRelationship BB
...
INSERT INTO DBO.AdvCustRelationship
  ...
  (	   [EntityKey]
   ...
   FROM RBL_TEMPDB.DBO.TempAdvCustRelationship T Where ISNULL(T.IsChanged,'U') IN ('N','C')
