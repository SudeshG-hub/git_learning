...
=============================================*/
create  PROCEDURE [PRO].[Final_AssetClass_Npadate_17022023]
...
/*------------------------------UPDATE UNIFORM ASSET CLASSIFICATION--------------------------------*/
 ...
 /* START PERCOLATION WORK -- 31082021 */
	...
	EXEC [PRO].[BuyoutDataProcessing] @TimeKey
	...
	FROM
		(		/* ADVANCE DATA */
			...
			UNION 
				SELECT UCIF_ID,ENBDAcNo,MAX(ISNULL(FinalAssetClassAlt_Key,1)) SYSASSETCLASSALT_KEY ,MIN(FinalNpaDt) SYSNPA_DT
				,'BUY' PercType
			FROM BuyoutFinalDetails A 
			INNER JOIN PRO.CUSTOMERCAL B ON A.CIFId = B.RefCustomerID
				WHERE  A.EffectiveFromTimeKey <=@TIMEKEY AND A.EffectiveToTimeKey >=@TIMEKEY
					AND ISNULL(FinalAssetClassAlt_Key,1)<>1 and PartnerDPD > 0 
			GROUP BY  UCIF_ID,ENBDAcNo
		)A
				...
				,A.DegReason=CASE WHEN A.SysAssetClassAlt_Key =1 AND B.SYSASSETCLASSALT_KEY >1 
								THEN  
									CASE WHEN B.PercType ='INV' THEN	'PERCOLATION BY INVESTMENT INVID '  + B.CustomerACID 
										 ...
										 WHEN B.PercType ='BUY' THEN	'PERCOLATION BY BUYOUT ACCOUNTID '  + B.CustomerACID  	
										ELSE A.DegReason
									END 
								ELSE  A.DegReason
							END				
	FROM PRO.CUSTOMERCAL A
		...
		,(CASE WHEN A.PercType = 'ADV' THEN STUFF((SELECT ', ' + B.CustomerACID
				...
				WHEN  A.PercType = 'DER' THEN STUFF((SELECT ', ' + B.CustomerACID
										...
										ORDER BY CustomerACID
										FOR XML PATH('')),1,1,'')
				WHEN  A.PercType = 'BUY' THEN STUFF((SELECT ', ' + B.CustomerACID
										from #TEMPTABLE_UCFIC1 B 
										WHERE B.PercType = 'BUY' and B.UCIF_ID = A.UCIF_ID
										ORDER BY CustomerACID
										FOR XML PATH('')),1,1,'')
										END) CustomerACID
				...
				,A.DegReason=CASE WHEN A.FinalAssetClassAlt_Key =1 AND D.SYSASSETCLASSALT_KEY >1 
								THEN  
									CASE WHEN D.PercType ='ADV' THEN	'PERCOLATION BY LOAN ACCOUNTID ' + D.CustomerACID 
										 ...
										 WHEN D.PercType ='BUY' THEN	'PERCOLATION BY BUYOUT ACCOUNTID '  + D.CustomerACID 
										ELSE A.DegReason
									END 
								ELSE  A.DegReason
							END
				...
				,A.DegReason=CASE WHEN A.FinalAssetClassAlt_Key =1 AND B.SYSASSETCLASSALT_KEY >1 
								THEN  
									CASE WHEN B.PercType ='ADV' THEN 'PERCOLATION BY LOAN ACCOUNTID ' + B.CustomerACID 
										 ...
										 WHEN B.PercType ='BUY' THEN 'PERCOLATION BY BUYOUT INVID ' + B.CustomerACID   	
										ELSE A.DegReason
									END 
								ELSE  A.DegReason
							END
	FROM CurDat.DerivativeDetail A
		...
		UPDATE A SET FinalAssetClassAlt_Key=B.SYSASSETCLASSALT_KEY
				...
				,A.DegReason=CASE WHEN A.FinalAssetClassAlt_Key =1 AND B.SYSASSETCLASSALT_KEY >1 
							...
							END
	FROM BuyoutFinalDetails A
	INNER JOIN PRO.CUSTOMERCAL C ON a.CIFId = C.RefCustomerID
		INNER JOIN (select A.UCIF_ID,A.PercType,A.SYSASSETCLASSALT_KEY,A.SYSNPA_DT
					,(CASE WHEN A.PercType = 'ADV' THEN STUFF((SELECT ', ' + B.CustomerACID
										from #TEMPTABLE_UCFIC1 B 
										WHERE B.PercType = 'ADV' and B.UCIF_ID = A.UCIF_ID
	...
	--DROP TABLE IF EXISTS #CTE_PERC
		...
		-- update A SET FinalAssetClassAlt_Key=FinalAssetClassAlt_Key
		-- FROM BuyoutFinalDetails A
		--where  A.EffectiveFromTimeKey<=@TIMEKEY AND A.EffectiveToTimeKey>=@TIMEKEY
		-- and FinalAssetClassAlt_Key>1
	...
	UPDATE A SET A.NPA_Reason=CASE WHEN  B.SYSASSETCLASSALT_KEY >1 
								THEN  
									CASE WHEN B.PercType ='INV' and A.NPA_Reason is  NULL THEN 'PERCOLATION BY INVESTMENT INVID '  + B.CustomerACID 
													...
													WHEN B.PercType ='BUY' and A.NPA_Reason is  NULL  THEN	'PERCOLATION BY BUYOUT ACCOUNTID '  + B.CustomerACID 
										ELSE A.NPA_Reason
									END 
								ELSE  A.NPA_Reason
							END				
	...
	UPDATE A SET A.DegReason=CASE WHEN  D.SYSASSETCLASSALT_KEY >1 
								THEN  
									CASE WHEN D.PercType ='ADV' and A.DegReason not like '%PERC%'  THEN	ISNULL(A.DegReason,'') + ',PERCOLATION BY LOAN ACCOUNTID ' + D.CustomerACID 
										 ...
										 WHEN D.PercType ='BUY' and A.DegReason not like '%PERC%' THEN	ISNULL(A.DegReason,'') + ',PERCOLATION BY BUYOUT ACCOUNTID '  + D.CustomerACID  	
										ELSE A.DegReason
									END 
								ELSE  A.DegReason
							END
	...
	UPDATE A SET A.DegReason=CASE WHEN  B.SYSASSETCLASSALT_KEY >1 
								THEN  
									CASE WHEN B.PercType ='ADV' and A.DegReason not like '%PERC%' THEN ISNULL(A.DegReason,'') + ',PERCOLATION BY LOAN ACCOUNTID ' + B.CustomerACID 
										 ...
										 WHEN B.PercType ='BUY' and A.DegReason not like '%PERC%' THEN ISNULL(A.DegReason,'') + ',PERCOLATION BY BUYOUT INVID ' + B.CustomerACID  
										ELSE A.DegReason
									END 
								ELSE  A.DegReason
							END
	FROM CurDat.DerivativeDetail A
		INNER JOIN (select A.UCIF_ID,A.PercType,A.SYSASSETCLASSALT_KEY,A.SYSNPA_DT
					...
					WHEN  A.PercType = 'BUY' THEN STUFF((SELECT ', ' + B.CustomerACID
										...
										END) CustomerACID
									FROM #CTE_PERC A									
									GROUP BY A.UCIF_ID,A.PercType,A.SYSASSETCLASSALT_KEY,A.SYSNPA_DT) B ON A.UCIC_ID =B.UCIF_ID and b.PercType IN ('adv','INV','buy')
		AND A.EffectiveFromTimeKey<=@tIMEKEY AND A.EffectiveToTimeKey>=@tIMEKEY
