...
@MenuID INT=24747,  
@UserLoginId  VARCHAR(20)='operater1',  
@Timekey INT=26479
,@filepath VARCHAR(MAX) ='UCIClvlInvstDerivMOCUpload (3).xlsx'  
WITH RECOMPILE  
AS  
...
END
  ------------------------------------------------------------------------------  
	...
	SET  
		...
		,ErrorinColumn='Sl.No.,UCIC ID,AssetClass,NPIDate,SecurityValue,AdditionalProvision%,MOCSource,MOCType,MOCReason'    
		,Srnooferroneousrows=''
...
FROM CalypsoUploadCustMocUpload V  
 ...
 IF OBJECT_ID('TempDB..#tmp') IS NOT NULL DROP TABLE #tmp; 
  ...
  Select  ROW_NUMBER() OVER(ORDER BY  CONVERT(INT,Entity_Key) ) RecentRownumber,Entity_Key,UCICID  into #tmp from CalypsoUploadCustMocUpload
 ...
 Select @Count=Count(*) from #tmp
   ...
   SET @UCICID=''
     While(@I<=@Count)
               BEGIN 
			      Select @UCICID =UCICID,@Entity_Key=Entity_Key  from #tmp where RecentRownumber=@I 
							order By Entity_Key
