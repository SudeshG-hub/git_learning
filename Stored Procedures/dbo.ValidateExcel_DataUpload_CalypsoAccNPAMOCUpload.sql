...
BEGIN
  ...
  IF NOT (EXISTS (SELECT * FROM [CalypsoAccountLvlMOCDetails_stg] where filname=@FilePathUpload))
...
-- ----SELECT * FROM CalypsoUploadAccMOCPool
 ...
 AND V.InvestmentIDDerivativeRefNo NOT IN(SELECT invid FROM InvestmentBasicDetail
								WHERE EffectiveFromTimeKey<=@Timekey AND EffectiveToTimeKey>=@Timekey
					union			SELECT DerivativeRefNo FROM CurDat.DerivativeDetail
								WHERE EffectiveFromTimeKey<=@Timekey AND EffectiveToTimeKey>=@Timekey
						 )
