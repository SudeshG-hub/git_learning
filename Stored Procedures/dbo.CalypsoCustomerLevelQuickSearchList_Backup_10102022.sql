...
CREATE PROC [dbo].[CalypsoCustomerLevelQuickSearchList_Backup_10102022]
--declare
	  ...
	  begin
		...
		select distinct T.CustomerId
				 ...
				 from CalypsoCustomerLevelMOC_Mod A
				  --inner join Customerlevelmoc_mod B ON B.CustomerID=A.RefCustomerID
				  INNER JOIN curdat.DerivativeDetail  T  ON T.CustomerId=@CustomerId  AND                                                   
...
END
		...
		select distinct T.IssuerID CustomerId
				 ...
				 from CalypsoCustomerLevelMOC_Mod A
				  --inner join Customerlevelmoc_mod B ON B.CustomerID=A.RefCustomerID
				 INNER JOIN InvestmentIssuerDetail   T  ON T.Issuerid=@customerid AND  T.EffectiveFromTimeKey<=@Timekey
					   AND T.EffectiveToTimeKey >= @Timekey 
				 where A.EffectiveFromTimeKey<=@Timekey
					   AND A.EffectiveToTimeKey >= @Timekey 
 ...
 ELSE
	   ...
	   PRINT 'Sac1'
		...
		select distinct T.IssuerID CustomerId
				 ...
				 from CalypsoInvMOC_ChangeDetails A
				  --inner join Customerlevelmoc_mod B ON B.CustomerID=A.RefCustomerID
				  INNER  JOIN InvestmentIssuerDetail  T  ON T.IssuerId=@customerid
				 AND  T.EffectiveFromTimeKey<=@Timekey
					   AND T.EffectiveToTimeKey >= @Timekey 
					   AND T.IssuerId=@CustomerId
				 where A.EffectiveFromTimeKey<=@Timekey
