...
----Select * 
from ENBD_TEMPDB.DBO.TempAdvAcOtherFinancialDetail A
...
INNER JOIN ENBD_TEMPDB.DBO.TempAdvAcOtherFinancialDetail AS T
...
AND NOT EXISTS (SELECT 1 FROM ENBD_TEMPDB.DBO.TempAdvAcOtherFinancialDetail BB
...
SELECT @EntityKey=MAX(EntityKey) FROM  ENBD_MISDB.[dbo].[AdvAcOtherFinancialDetail] 
...
SET TEMP.EntityKey=ACCT.EntityKey
 FROM ENBD_TEMPDB.DBO.[TempAdvAcOtherFinancialDetail] TEMP
INNER JOIN (SELECT AccountEntityId,(@EntityKey + ROW_NUMBER()OVER(ORDER BY (SELECT 1))) EntityKey
			FROM ENBD_TEMPDB.DBO.[TempAdvAcOtherFinancialDetail]
...
,PenalUnAppliedIntAmount
,PenalInterestOverDueSInceDt
	FROM ENBD_TEMPDB.DBO.TempAdvAcOtherFinancialDetail T Where ISNULL(T.IsChanged,'U') IN ('N','C')
