...
----Select * 
from ENBD_TEMPDB.DBO.TempInvestmentBasicDetail A
...
INNER JOIN ENBD_TEMPDB.DBO.TempInvestmentBasicDetail AS T
...
AND NOT EXISTS (SELECT 1 FROM ENBD_TEMPDB.DBO.TempInvestmentBasicDetail BB
...
SELECT @EntityKey=MAX(EntityKey) FROM  ENBD_MISDB.[dbo].[InvestmentBasicDetail] 
...
SET TEMP.EntityKey=ACCT.EntityKey
 FROM ENBD_TEMPDB.DBO.[TempInvestmentBasicDetail] TEMP
INNER JOIN (SELECT InvEntityId,(@EntityKey + ROW_NUMBER()OVER(ORDER BY (SELECT 1))) EntityKey
			FROM ENBD_TEMPDB.DBO.[TempInvestmentBasicDetail]
			WHERE EntityKey=0 OR EntityKey IS NULL)ACCT ON  Temp.InvEntityId=ACCT.InvEntityId
Where Temp.IsChanged in ('N','C')


	--IF (SELECT COUNT(1) FROM   ENBD_TEMPDB.dbo.TempInvestmentBasicDetail T Where ISNULL(T.IsChanged,'U') IN ('N','C')) > 0
	--BEGIN
...
SELECT
      ...
      ,[ApprovedBy]
      ,[DateApproved]
	FROM ENBD_TEMPDB.dbo.TempInvestmentBasicDetail T 
