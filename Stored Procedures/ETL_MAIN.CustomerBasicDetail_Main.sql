...
from ENBD_TEMPDB.DBO.TempCustomerBasicDetail A
...
INNER JOIN ENBD_TEMPDB.DBO.TempCustomerBasicDetail AS T
...
AND NOT EXISTS (SELECT 1 FROM ENBD_TEMPDB.DBO.TempCustomerBasicDetail BB
...
SELECT @Customer_Key=MAX(Customer_Key) FROM  ENBD_MISDB.[dbo].[CustomerBasicDetail] 
...
SET TEMP.Customer_Key=ACCT.Customer_Key
 FROM ENBD_TEMPDB.DBO.[TempCustomerBasicDetail] TEMP
INNER JOIN (SELECT CustomerId,(@Customer_Key + ROW_NUMBER()OVER(ORDER BY (SELECT 1))) Customer_Key
			FROM ENBD_TEMPDB.DBO.[TempCustomerBasicDetail]
...
SELECT
      ...
      ,[Remark]
      ,[SourceSystemAlt_Key]
		   FROM ENBD_TEMPDB.dbo.TempCustomerBasicDetail T Where ISNULL(T.IsChanged,'U') IN ('N','C')
