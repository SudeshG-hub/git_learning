...
CREATE PROCEDURE [dbo].[ValidateExcel_DataUpload_CalypsoCustMocUpload_07102022]  
@MenuID INT=24747,  
@UserLoginId  VARCHAR(20)='testuser8',  
@Timekey INT=26479
...
END
  ------------------------------------------------------------------------------  
	...
	SET  
		...
		,ErrorinColumn='Sl.No.,Customer ID,AssetClass,NPIDate,SecurityValue,AdditionalProvision%,MOCSource,MOCType,MOCReason'    
		,Srnooferroneousrows=''
...
FROM CalypsoUploadCustMocUpload V  
 ...
 IF OBJECT_ID('TempDB..#tmp') IS NOT NULL DROP TABLE #tmp; 
  ...
  Select  ROW_NUMBER() OVER(ORDER BY  CONVERT(INT,Entity_Key) ) RecentRownumber,Entity_Key,CustomerID  into #tmp from CalypsoUploadCustMocUpload
   ...
   SET @RefCustomerID=''
     While(@I<=@Count)
               BEGIN 
				...
				Select      @CustomerIDFound=(select Count(1)
				from InvestmentBasicDetail  A Where  RefIssuerID =@RefCustomerID
				and EffectiveFromTimeKey <= @TimeKey AND EffectiveToTimeKey >= @TimeKey
				UNION
				select Count(1)
				from curdat.DerivativeDetail  A Where  CustomerID =@RefCustomerID
				and EffectiveFromTimeKey <= @TimeKey AND EffectiveToTimeKey >= @TimeKey)
...
UPDATE CalypsoUploadCustMocUpload
	SET  
        ErrorMessage= CASE WHEN ISNULL(ErrorMessage,'')='' THEN 'You can not upload CustomerID. For these CustomerID MOC is not Processed'     
						ELSE ErrorMessage+','+SPACE(1)+ 'You can not upload CustomerID. For these CustomerID MOC is not Processed'     END
		,ErrorinColumn=CASE WHEN ISNULL(ErrorinColumn,'')='' THEN 'CustomerID' ELSE ErrorinColumn +','+SPACE(1)+  'CustomerID' END  
		,Srnooferroneousrows=V.SlNo
--						
 FROM CalypsoUploadCustMocUpload V  
 WHERE ISNULL(V.CustomerId,'')<>''
AND ISNULL(V.CustomerId,'') in ( Select B.CustomerId from MOC_ChangeDetails A
...
SELECT * into AssetClassData  FROM(
 ...
 FROM CalypsoUploadCustMocUpload V  
 WHERE    ISDATE(NPIDate)=0  AND  ISNULL(NPIDate,'')<>'' 
 ...
 FROM CalypsoUploadCustMocUpload V  
 WHERE ISNULL(AssetClass,'') IN('NPA') AND (NPIDate)=''
 ...
 FROM CalypsoUploadCustMocUpload V  
 WHERE (Case When ISDATE(NPIDate)=1 Then Case When Cast(NPIDate as date)>Cast(GETDATE() as Date) Then 1 Else 0 END END)=1
...
SELECT * into MocSourceData  FROM(
 ...
 ROW ,MOCSOURCE FROM CalypsoUploadCustMocUpload
)X
 WHERE ROW=1
