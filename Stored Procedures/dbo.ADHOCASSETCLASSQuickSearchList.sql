...
SET @PageTo = @pageSize*@newPage  
	...
	Set @Timekey =(Select Timekey from SysDataMatrix where CurrentStatus='C') 
	 	...
	 	IF ((@CustomerId ='' or (@CustomerId is null)) AND   (@UCICID =''or (@UCICID is null) ) AND (@operationflag not in(16,20)))
		...
		select distinct A.CustomerID CustomerId,CH.CustomerName
				...
				ON A.CustomerID=CH.CustomerID and CH.EffectiveFromTimeKey<=@Timekey
				 ...
				 where A.EffectiveFromTimeKey<=@Timekey
					...
					AND ISNULL(A.AuthorisationStatus,'A') IN ('A')
			UNION
	...
	END
	        ELSE
				BEGIN
				 ...
				 WHERE ((A.CUSTOMERid=@CustomerId)   or  (A.UCIF_ID=@UCICID))
