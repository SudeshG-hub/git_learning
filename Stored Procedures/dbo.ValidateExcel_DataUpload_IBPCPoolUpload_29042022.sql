...
CREATE PROCEDURE [dbo].[ValidateExcel_DataUpload_IBPCPoolUpload_29042022]  
@MenuID INT=10,  
 ...
 */
 ...
 AND LEN(PoolType)>20
  ...
  UPDATE UploadIBPCPool
	SET  
        ErrorMessage=CASE WHEN ISNULL(ErrorMessage,'')='' THEN 'Invalid PoolType.  Please check the values With Risk or With out Risk  and upload again'     
						ELSE ErrorMessage+','+SPACE(1)+'Invalid PoolType.  Please check the values and upload again'     END
		,ErrorinColumn=CASE WHEN ISNULL(ErrorinColumn,'')='' THEN 'PoolType' ELSE   ErrorinColumn +','+SPACE(1)+'PoolType' END       
		,Srnooferroneousrows=V.SrNo
   ...
   FROM UploadIBPCPool V  
 WHERE PoolType  NOT in  ('With Risk' , 'With out Risk')
...
--								--),1,1,'')   
 ...
 FROM UploadIBPCPool V  
 WHERE (ISNUMERIC(IBPCExposureinRs)=0 AND ISNULL(IBPCExposureinRs,'')<>'') OR 
 ISNUMERIC(IBPCExposureinRs) LIKE '%^[0-9]%'
 PRINT 'INVALID' 
...
-- WHERE ISNULL(DateofIBPCreckoning,'')<>'' AND (Cast(DateofIBPCreckoning as date)>Cast(DateofIBPCmarking as Date) OR Cast(DateofIBPCreckoning as Date)>Cast(MaturityDate as Date))
 ...
 UPDATE UploadIBPCPool
	SET  
        ErrorMessage=CASE WHEN ISNULL(ErrorMessage,'')='' THEN 'DateofIBPCmarking Can not be Blank . Please enter the DateofIBPCmarking and upload again'     
						ELSE ErrorMessage+','+SPACE(1)+ 'DateofIBPCmarking Can not be Blank. Please enter the DateofIBPCmarking and upload again'      END
 ...
 FROM UploadIBPCPool V  
 WHERE ISNULL(DateofIBPCmarking,'')='' 
