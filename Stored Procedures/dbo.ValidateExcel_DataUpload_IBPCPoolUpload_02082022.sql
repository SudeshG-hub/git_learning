...
CREATE PROCEDURE [dbo].[ValidateExcel_DataUpload_IBPCPoolUpload_02082022]  
@MenuID INT=10,  
 ...
 */
 ...
 AND LEN(PoolType)>20
  ...
  UPDATE UploadIBPCPool
	SET  
        ErrorMessage=CASE WHEN ISNULL(ErrorMessage,'')='' THEN 'Invalid PoolType.  Please check the values With Risk or Without Risk  and upload again'     
						ELSE ErrorMessage+','+SPACE(1)+'Invalid PoolType.  Please check the values and upload again'     END
		,ErrorinColumn=CASE WHEN ISNULL(ErrorinColumn,'')='' THEN 'PoolType' ELSE   ErrorinColumn +','+SPACE(1)+'PoolType' END       
		,Srnooferroneousrows=V.SrNo
   ...
   FROM UploadIBPCPool V  
 WHERE PoolType  NOT in  ('With Risk' , 'Without Risk')
