...
----Select * 
from ENBD_TEMPDB.DBO.TempAdvFacCCDetail A
...
INNER JOIN ENBD_TEMPDB.DBO.TempAdvFacCCDetail AS T
...
AND NOT EXISTS (SELECT 1 FROM ENBD_TEMPDB.DBO.TempAdvFacCCDetail BB
...
SELECT @EntityKey=MAX(EntityKey) FROM  ENBD_MISDB.[dbo].[AdvFacCCDetail] 
...
SELECT
	  ...
	  ,StockStmtDt
		   FROM ENBD_TEMPDB.dbo.TempAdvFacCCDetail T Where ISNULL(T.IsChanged,'U') IN ('N','C')
