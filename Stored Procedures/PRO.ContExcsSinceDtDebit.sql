...
update Pro.ContExcsSinceDtDebitAccountCal set EffectiveToTimekey = @Timekey - 1 
...
IF OBJECT_ID('TEMPDB..#ContExcsSinceDtDebitAccountCal') IS NOT NULL
   DROP TABLE #ContExcsSinceDtDebitAccountCal
      ...
      SELECT C.CustomerAcID INTO #ContExcsSinceDtDebitAccountCal  FROM 
DBO.AdvAcBalanceDetail  A
INNER JOIN [dbo].[AdvAcBasicDetail] B ON A.AccountEntityId=B.AccountEntityId
INNER JOIN ENBD_STGDB..FINACLE_ACCOUNT_STG C ON C.CustomerAcID=B.CustomerACID
...
INNER JOIN ENBD_STGDB..FINACLE_ACCOUNT_STG C 
