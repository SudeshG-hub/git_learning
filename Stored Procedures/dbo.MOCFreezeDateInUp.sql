...
SET @MOC_Initialized_Date=(SELECT ExtDate FROM SYSDATAMATRIX WHERE isnull(MOC_FROZEN,'N')='N'
	...
	PRINT 1
		...
		IF EXISTS(				                
					...
					UNION
					SELECT  1 FROM MOCFreezeDetails_Mod  WHERE (EffectiveFromTimeKey <=@TimeKey AND EffectiveToTimeKey >=@TimeKey)
