...
from ENBD_TEMPDB.DBO.TempAdVAcBalanceDetail A
...
INNER JOIN ENBD_TEMPDB.DBO.TempAdVAcBalanceDetail AS T
...
AND NOT EXISTS (SELECT 1 FROM ENBD_TEMPDB.DBO.TempAdVAcBalanceDetail BB
...
SELECT @EntityKey=MAX(EntityKey) FROM  ENBD_MISDB.[dbo].[AdvAcBalanceDetail] 
...
SET TEMP.EntityKey=ACCT.EntityKey
 FROM ENBD_TEMPDB.DBO.[TempAdvAcBalanceDetail] TEMP
INNER JOIN (SELECT AccountEntityId,(@EntityKey + ROW_NUMBER()OVER(ORDER BY (SELECT 1))) EntityKey
			FROM ENBD_TEMPDB.DBO.[TempAdvAcBalanceDetail]
...
SELECT
			...
			,SourceAssetClass
			,SourceNpaDate
		   FROM ENBD_TEMPDB.DBO.TempAdVAcBalanceDetail T Where ISNULL(T.IsChanged,'U') IN ('N','C')
