			...
			IF(@OperationFlag not in ( 16,17,20))
			 ...
			 IF OBJECT_ID('TempDB..#temp') IS NOT NULL
              ...
              (
                     ...
                     FROM		[dbo].[IBPCACFlaggingDetail] A Inner join DIMSOURCEDB ds on ds.SourceAlt_Key=A.SourceAlt_Key
					 WHERE  A.EffectiveFromTimeKey <= @TimeKey
                           AND A.EffectiveToTimeKey >= @TimeKey
                           AND ISNULL(A.AuthorisationStatus,'A') ='A'
						   And Not exists(Select 1 from IBPCFinalPoolDetail B 
					...
					SELECT 
							...
							,A.CustomerID
							,B.CustomerName
							,(CASE WHEN Action = 'A' THEN 'Y' ELSE 'N' END) FlagAlt_Key
							,A.PoolID
							,A.PoolName
							,A.OSBalance
							,A.POS
							...
							,A.PoolType
							,Case when isnull(A.MaturityDate,'')='' then Null else convert(Varchar(10),MaturityDate,103) ENd MaturityDate
							,Case when isnull(A.DateofIBPCmarking,'')='' then Null else convert(Varchar(10),DateofIBPCmarking,103) ENd IBPCMarkingDate
                     ...
                     FROM		[dbo].[IBPCPoolDetail] A 
					 INNER JOIN CustomerBasicDetail B ON A.CustomerID = B.CustomerId
					 and B.EffectiveFromTimeKey <= @TimeKey and B.EffectiveToTimeKey >= @Timekey
					 --Inner join DIMSOURCEDB ds on ds.SourceAlt_Key=A.SourceSyst
					 WHERE  A.EffectiveFromTimeKey <= @TimeKey
                           AND A.EffectiveToTimeKey >= @TimeKey
                           AND ISNULL(A.AuthorisationStatus,'A') ='A'
						   And Not exists(Select 1 from IBPCACFlaggingDetail_Mod B 
                     ...
                     SELECT A.AccountFlagAlt_Key
							...
							,ISNULL(A.DateApprovedFirstLevel,A.DateModified) as ModAppDate
							 ,A.Changefields
                     FROM		[dbo].[IBPCACFlaggingDetail_Mod] A Inner join DIMSOURCEDB ds on ds.SourceAlt_Key=A.SourceAlt_Key
					 WHERE A.EffectiveFromTimeKey <= @TimeKey
                           AND A.EffectiveToTimeKey >= @TimeKey
						    And Not exists(Select 1 from IBPCFinalPoolDetail B 
						    WHERE   ISNULL(B.AuthorisationStatus,'A') IN ('A')
