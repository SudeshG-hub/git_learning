...
CREATE PROCEDURE [PRO].[InsertDataforAssetClassficationENBD_MOC_NEW]
@TIMEKEY INT
AS
BEGIN
...
DROP TABLE IF EXISTS #Moc_Cust
		...
		SELECT UcifEntityID,CBD.UCIF_ID,  MAX(AssetClassAlt_Key) SysAssetClassAlt_Key, min(NPA_Date) SysNPA_Dt,MIN(MOC_ExpireDate) MOC_ExpireDate,'N' AssetClassChanged
			INTO #MOC_DATA
		  FROM MOC_ChangeDetails A
			...
			GROUP BY UcifEntityID,CBD.UCIF_ID

		DROP TABLE IF EXISTS #CUST_HIST
		SELECT B.UcifEntityID ,A.UCIF_ID,MAX(B.SysAssetClassAlt_Key) SysAssetClassAlt_Key, MIN(B.SysNPA_Dt) SysNPA_Dt
			INTO #CUST_HIST
		FROM #MOC_DATA A
		...
		GROUP BY  B.UcifEntityID,A.UCIF_ID
	...
	/* CHECK AND UPDATE ASSETCLASS FOR COMMON UCIC_ID IN ADVANCE AND INVESTMENT */
		UPDATE A
			SET A.SysAssetClassAlt_Key=CASE WHEN A.SysAssetClassAlt_Key>B.AssetClassAlt_Key THEN A.SysAssetClassAlt_Key ELSE B.AssetClassAlt_Key END
				,A.SysNPA_Dt=CASE WHEN A.SysNPA_Dt<=B.NPA_Date THEN A.SysNPA_Dt ELSE B.NPA_Date END
		FROM #MOC_DATA A
			INNER JOIN CalypsoDervMOC_ChangeDetails B
				ON EffectivefromTimeKey<=@TimeKey and EffectiveToTimeKey>=@TimeKey
				AND A.UCIF_ID=B.UCICID

	/* CHECK AND UPDATE ASSETCLASS FOR COMMON UCIC_ID IN ADVANCE AND DERIVATIVE */

		UPDATE A
			SET A.SysAssetClassAlt_Key=CASE WHEN A.SysAssetClassAlt_Key>B.AssetClassAlt_Key THEN A.SysAssetClassAlt_Key ELSE B.AssetClassAlt_Key END
				,A.SysNPA_Dt=CASE WHEN A.SysNPA_Dt<=B.NPA_Date THEN A.SysNPA_Dt ELSE B.NPA_Date END
		FROM #MOC_DATA A
			INNER JOIN CalypsoInvMOC_ChangeDetails B
				ON EffectivefromTimeKey<=@TimeKey and EffectiveToTimeKey>=@TimeKey
				AND A.UCIF_ID=B.UCICID


	
	/* ADD COMMON UCIC ID (MOC DONE IN INVESTMENT/DERIVATIVE) FOR -CUSTOMER AND ACCOUNT DATA PROCESSING - */	
	 ;WITH CTE_A
	 AS(
		SELECT UCICID,AssetClassAlt_Key,NPA_date FROM CalypsoDervMOC_ChangeDetails 
			WHERE EffectiveFromTimeKey<=@TimeKey and EffectiveToTimeKey>=@TimeKey
		UNION 
		SELECT UCICID,AssetClassAlt_Key,NPA_date  FROM  CalypsoInvMOC_ChangeDetails	
		WHERE EffectiveFromTimeKey<=@TimeKey and EffectiveToTimeKey>=@TimeKey
		)

		INSERT INTO #MOC_DATA
		SELECT UCICID,MAX(AssetClassAlt_Key) AssetClassAlt_Key,MIN(NPA_date) NPA_date 
		FROM CTE_A A
			LEFT JOIN #MOC_DATA B
...
SELECT	
	...
	FROM CURDAT.InvestmentFinancialDetail A
		...
		INNER JOIN CURDAT.InvestmentIssuerDetail ISR
			on ISR.IssuerEntityId=INB.IssuerEntityId
			and ISR.EffectiveFromTimeKey<=@TIMEKEY and ISR.EffectiveToTimeKey>=@TIMEKEY
		INNER JOIN #MOC_DATA aa
					on AA.UCIF_ID=ISR.UcifId
		LEFT JOIN PreMoc.InvestmentFinancialDetail b
			on a.InvEntityId=b.InvEntityId
			and b.EffectiveFromTimeKey=@TIMEKEY and b.EffectiveToTimeKey=@TIMEKEY
		WHERE B.InvEntityId IS NULL
