...
CREATE PROCEDURE [dbo].[Rpt-039A_19-10-2022]	
    @TimeKey AS INT,
	@SelectReport AS INT
...
CONVERT(VARCHAR(15),IFD.NPIDt,103)                AS SubSTD,
	  
CONVERT(VARCHAR(20),DBTDate,103)                  AS DBTDate1,
''								                  AS DBTDate2,
''								                  AS DBTDate3,
...
CASE WHEN FD.AuthorisationStatus='A' 
     THEN 'Y' 
	 ELSE 'N' 
	 END									   AS RFAFraudFlag,
...
0												  AS ApprRV,
0												  AS ProvDFV,
ISNULL(DPS.AdditionalprovisionRBINORMS,0)		  AS AddlProvision,
0												  AS WriteOffAmount,
...
LEFT JOIN	 Fraud_Details		FD							ON  IID.IssuerEntityID=FD.CustomerEntityid 
...
C.SrcSysClassName                                                        AS SubAssetClass,
CONVERT(VARCHAR(15),Derivative.NPIDt,103)                                AS SubSTD,	  
CONVERT(VARCHAR(20),DBTDate,103)                                         AS DBTDate1,
''														                  AS DBTDate2,
''														                  AS DBTDate3,
ISNULL(ProvisionSecured,0)                                               AS ProvisionPerSecured,
...
LEFT JOIN Fraud_Details		FD					  ON  Derivative.DerivativeEntityID=FD.ACCOUNTENTITYID 
													  AND FD.EffectiveFromTimeKey<=@TimeKey 
                                                      AND FD.EffectiveToTimeKey>=@TimeKey
