...
CREATE PROCEDURE [dbo].[Rpt-029_26-05-2022]
      @TimeKey AS INT,
	  @Cost    AS FLOAT
AS
...
-------------
,CASE WHEN A2.AssetClassName ='STANDARD' AND Y1.AssetClassName <>'STANDARD' THEN
                                           (CASE WHEN (ISNULL(B.NetBalance,0) - ISNULL(Y1.netBalance,0)) < 0 
                                                  THEN 0 
	                                              ELSE (ISNULL(B.NetBalance,0) - ISNULL(Y1.netBalance,0)) 
	                                              END)/@Cost    
ELSE 0 END AS NPAIncrease

,CASE WHEN A2.AssetClassName ='STANDARD' AND Y1.AssetClassName <>'STANDARD' THEN 
(CASE WHEN (ISNULL(B.NetBalance,0) - ISNULL(Y1.netBalance,0)) >= 0 
       THEN 0 
	   ELSE (ISNULL(B.NetBalance,0) - ISNULL(Y1.netBalance,0)) 
	   END)/@Cost   ELSE 0 END                                                  AS NPADecrease

,CASE WHEN A2.AssetClassName ='STANDARD' AND Y1.AssetClassName <>'STANDARD' THEN 
(CASE WHEN (ISNULL(B.TotalProvision,0) - ISNULL(Y1.TotalProvision,0)) < 0 
       THEN 0 
	   ELSE (ISNULL(B.TotalProvision,0) - ISNULL(Y1.TotalProvision,0)) 
	   END)/@Cost   ELSE 0 END                                                   AS ProvisionIncrease

,CASE WHEN A2.AssetClassName ='STANDARD' AND Y1.AssetClassName <>'STANDARD' THEN 
(CASE WHEN (ISNULL(B.TotalProvision,0) - ISNULL(Y1.TotalProvision,0)) >= 0 
       THEN 0 
	   ELSE (ISNULL(B.TotalProvision,0) - ISNULL(Y1.TotalProvision,0)) 
	   END)/@Cost  ELSE 0 END                                                    AS ProvisionDecrease

,CASE WHEN A2.AssetClassName ='STANDARD' AND Y1.AssetClassName <>'STANDARD' THEN
        (CASE WHEN ((ISNULL(B.NetBalance,0)-ISNULL(B.TotalProvision,0)) - ISNULL(Y1.NetNPA,0)) < 0 
       THEN 0 
	   ELSE ((ISNULL(B.NetBalance,0)-ISNULL(B.TotalProvision,0)) - ISNULL(Y1.NetNPA,0)) 
	   END)/@Cost                                                     
	   ELSE 0 END AS NetNPAIncrease

,CASE WHEN A2.AssetClassName ='STANDARD' AND Y1.AssetClassName <>'STANDARD' THEN 
(CASE WHEN ((ISNULL(B.NetBalance,0)-ISNULL(B.TotalProvision,0)) - ISNULL(Y1.NetNPA,0)) >= 0 
       THEN 0 
	   ELSE ((ISNULL(B.NetBalance,0)-ISNULL(B.TotalProvision,0)) - ISNULL(Y1.NetNPA,0)) 
	   END)/@Cost  ELSE 0 END                                                    AS NetNPAnDecrease
