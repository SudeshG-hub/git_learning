...
FROM ENBD_TEMPDB.DBO.TempAdvCustOtherDetail A
...
INNER JOIN ENBD_TEMPDB.DBO.TempAdvCustOtherDetail AS T
...
AND NOT EXISTS (SELECT 1 FROM ENBD_TEMPDB.DBO.TempAdvCustOtherDetail BB
...
SELECT @EntityKey=MAX(EntityKey) FROM  ENBD_MISDB.[dbo].[AdvCustOtherDetail] 
...
SET TEMP.EntityKey=ACCT.Customer_Key
 FROM ENBD_TEMPDB.DBO.[TempAdvCustOtherDetail] TEMP
INNER JOIN (SELECT CUSTOMERENTITYID,(@EntityKey + ROW_NUMBER()OVER(ORDER BY (SELECT 1))) Customer_Key
			FROM ENBD_TEMPDB.DBO.[TempAdvCustOtherDetail]
...
SELECT
      ...
      ,ReasonsNonCoOperativeBorrower
      ,StatusNonCoOperativeBorrower
  FROM ENBD_TEMPDB.DBO.TempAdvCustOtherDetail T Where ISNULL(T.IsChanged,'U') IN ('N','C')
