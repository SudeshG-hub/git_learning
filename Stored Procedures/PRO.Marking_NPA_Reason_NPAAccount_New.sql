...
=============================================*/
CREATE PROCEDURE [PRO].[Marking_NPA_Reason_NPAAccount_New]
@TIMEKEY INT
with recompile
AS
BEGIN
...
FROM PRO.AccountCal A INNER JOIN PRO.CustomerCal B ON A.CustomerEntityID =B.CustomerEntityID
WHERE (B.FlgProcessing='N')  AND ( isnull(FinalAssetClassAlt_Key,1)<>1 AND (A.DPD_INTSERVICE>0 OR A.InttServiced='N'))
...
UPDATE A SET A.NPA_Reason= (CASE WHEN isnull(A.NPA_Reason,'') <> '' THEN isnull(A.NPA_Reason,'')+', DEGRADE BY OVERDUE' ELSE  'DEGRADE BY OVERDUE' END)
FROM PRO.AccountCal A INNER JOIN PRO.CustomerCal B ON A.CustomerEntityID =B.CustomerEntityID
...
IF OBJECT_ID('TEMPDB..#TEMPTABLE_PERCOLATION') IS NOT NULL
 ...
 --   DROP TABLE #TEMPTABLE_PERCOLATIONFreshSlipage
	...
	--ORDER BY A.REFCUSTOMERID
	 ...
	 UPDATE A SET NPA_Reason=CASE WHEN ISNULL(A.NPA_Reason,'') <> '' THEN ISNULL(A.NPA_Reason,'')+', Link By AccountId' + ' ' + B.CustomerAcID ELSE 'Link By AccountId' + ' ' + B.CustomerAcID END 
	 FROM PRO.AccountCal A 
