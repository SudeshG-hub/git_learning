...
CREATE PROC [dbo].[CalypsoCustomerLevelInUp_19102022]
...
IF(@MocStatus='InProgress')
  ...
  End
	...
	Select  @CustomerEntityID= IssuerEntityId  from InvestmentIssuerDetail 
	                          where UcifId=@UCICID
  ...
  End
	...
	IF @OperationFlag =1 AND @AuthMode ='Y' -- ADD
		BEGIN
					 ...
					 GOTO CalypsoMOCcustomerDateMaster_Insert
					CalypsoMOCcustomerDateMaster_Insert_Add:
			END
			...
			BEGIN
				...
				ELSE ---IF DATA IS AVAILABLE IN MAIN TABLE
					...
					GOTO CalypsoMOCcustomerDateMaster_Insert
					CalypsoMOCcustomerDateMaster_Insert_Edit_Delete:
				END
		...
		-- DELETE WITHOUT MAKER CHECKER
						...
						UPDATE CalypsoCustomerLevelMOC_Mod SET
									ModifiedBy =@Modifiedby 
									,DateModified =@DateModified 
			...
			IF @AuthMode='Y'  
				BEGIN
					...
					UPDATE CalypsoCustomerLevelMOC_Mod
						...
						WHERE (EffectiveFromTimeKey<=@Timekey AND EffectiveToTimeKey >=@Timekey)
						AND  CustomerEntityID =@CustomerEntityID
						AND AuthorisationStatus='A'	and Entity_Key=@ExEntityKey
...
CalypsoMOCcustomerDateMaster_Insert:
IF @ErrorHandle=0
	BEGIN
	...
	SET @Parameter3 = (select STUFF((	SELECT Distinct ',' +ChangeField
											from CalypsoCustomerLevelMOC_Mod where   CustomerEntityID=@CustomerEntityID
...
from (
 ...
 SET @FinalParameter3 = (select STUFF((	SELECT Distinct ',' + Value from #AA  for XML PATH('')),1,1,''))
		         ...
		         IF @OperationFlag =1 AND @AUTHMODE='Y'
					BEGIN
						PRINT 3
						GOTO CalypsoMOCcustomerDateMaster_Insert_Add
					END
				ELSE IF (@OperationFlag =2 OR @OperationFlag =3)AND @AUTHMODE='Y'
					BEGIN
						GOTO CalypsoMOCcustomerDateMaster_Insert_Edit_Delete
					END
