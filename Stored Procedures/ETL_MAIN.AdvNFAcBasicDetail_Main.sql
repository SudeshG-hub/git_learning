...
----Select * 
from ENBD_TEMPDB.DBO.TempAdvNFAcBasicDetail A
...
INNER JOIN ENBD_TEMPDB.DBO.TempAdvNFAcBasicDetail AS T
...
AND NOT EXISTS (SELECT 1 FROM ENBD_TEMPDB.DBO.TempAdvNFAcBasicDetail BB
...
SELECT @Ac_Key=MAX(Ac_Key) FROM  ENBD_MISDB.[dbo].[AdvNFAcBasicDetail] 
IF @Ac_Key IS NULL  
BEGIN
SET @Ac_Key=0
END
...
SET TEMP.Ac_Key=ACCT.Ac_Key
 FROM ENBD_TEMPDB.DBO.[TempAdvNFAcBasicDetail] TEMP
INNER JOIN (SELECT CustomerAcId,(@Ac_Key + ROW_NUMBER()OVER(ORDER BY (SELECT 1))) Ac_Key
			FROM ENBD_TEMPDB.DBO.[TempAdvNFAcBasicDetail]
...
,segmentcode
,SourceAlt_Key
FROM ENBD_TEMPDB.DBO.TempAdvNFAcBasicDetail T Where ISNULL(T.IsChanged,'U') IN ('N','C')
