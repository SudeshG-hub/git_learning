...
CREATE PROCEDURE [dbo].[ValidateExcel_DataUpload_StockStatementUpload_20042022]  
@MenuID INT=10,  
...
BEGIN
  ...
  -----------------SELECT * FROM StockStatement_stg
  ...
  ----update StockStatement_stg set filname='2ndlvlchecker_RestructuredAssetsUpload.xlsx'
BEGIN
print 'NO DATA'
...
--alter table select * from StockStatement_stg
...
----------------------------------------------
 ...
 WHERE ISNULL(StockStamentDt,'')=''
  ...
  FROM StockStatementUpload V  
 WHERE   ISDATE(StockStamentDt)=0 AND ISNULL(StockStamentDt,'')<>''
