...
CREATE PROCEDURE [dbo].[Rpt-028_31-05-2022]
      @TimeKey AS INT,
	  @Cost    AS FLOAT
AS
...
,CASE WHEN ISNULL(BAL.SignBalance,0)>0 THEN ISNULL(BAL.SignBalance,0)   
      ELSE 0
	  END AS CREDITBAL
...
,CASE WHEN C.AssetClassAlt_Key=1
      THEN (CASE WHEN B.SMA_Class='STD' THEN 'STD'
...
,ISNULL(FIN.PenalUnAppliedIntAmount,0)/@Cost                         AS PENAL_INTEREST
...
LEFT JOIN		(select Distinct SourceAlt_Key,AssetClassAlt_Key,(CASE WHEN AssetClassAlt_Key = 1 THEN 'STD' ELSE SrcSysClassCode END)SrcSysClassName ,----AssetClassMappingAlt_Key,
