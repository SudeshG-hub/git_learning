...
CREATE PROC [dbo].[Rpt-024_29-10-2022]
	@TimeKey  INT
	AS
 ...
 IF(OBJECT_ID('TEMPDB..#PRE_MOC')IS NOT NULL)
  ...
  SELECT H.CustomerID  
		...
		,CASE WHEN MOC.ChangeField=2		 THEN 'POS in Rs.'
              ...
              WHEN MOC.ChangeField=4		 THEN 'Restructure Flag'
              WHEN MOC.ChangeField=5		 THEN 'Restructure Date' 
              WHEN MOC.ChangeField=6		 THEN 'Fraud Flag'
              ...
              WHEN MOC.ChangeField=16		 THEN 'Unusual Bounce Flag'
              WHEN MOC.ChangeField=17		 THEN 'Unusual Bounce Date'
              WHEN MOC.ChangeField=18		 THEN 'Uncleared Effects Flag'
              WHEN MOC.ChangeField=19		 THEN 'Uncleared Effects Date'
		...
		,CASE WHEN MOC.ChangeField=2		 THEN CAST(ISNULL(AC.PrincOutStd,0)  AS VARCHAR(MAX))
              ...
              WHEN MOC.ChangeField=4		 THEN AC.FlgRestructure   
              WHEN MOC.ChangeField=5		 THEN CONVERT(VARCHAR(20),AC.RestructureDate,103) 
              WHEN MOC.ChangeField=6		 THEN AC.FlgFraud   
              WHEN MOC.ChangeField=7		 THEN CONVERT(VARCHAR(20),AC.FraudDate,103) 
              WHEN MOC.ChangeField=8		 THEN AC.FlgFITL 
              WHEN MOC.ChangeField=9		 THEN CAST(ISNULL(AC.DFVAmt,0) AS VARCHAR(MAX))
              WHEN MOC.ChangeField=10		 THEN AC.RePossession
              WHEN MOC.ChangeField=11		 THEN CONVERT(VARCHAR(20),AC.RepossessionDate,103) 
              WHEN MOC.ChangeField=12		 THEN AC.WeakAccount
              WHEN MOC.ChangeField=13		 THEN CONVERT(VARCHAR(20),AC.WeakAccountDate,103)
              WHEN MOC.ChangeField=14		 THEN AC.Sarfaesi
              WHEN MOC.ChangeField=15		 THEN CONVERT(VARCHAR(20),AC.SarfaesiDate,103)  
              WHEN MOC.ChangeField=16		 THEN AC.FlgUnusualBounce
              WHEN MOC.ChangeField=17		 THEN CONVERT(VARCHAR(20),AC.UnusualBounceDate,103)
              WHEN MOC.ChangeField=18		 THEN AC.FlgUnClearedEffect
              WHEN MOC.ChangeField=19		 THEN CONVERT(VARCHAR(20),AC.UnClearedEffectDate,103)  
              WHEN MOC.ChangeField=20		 THEN CAST(ISNULL(AddlProvisionPer,0)  AS VARCHAR(MAX))
              WHEN MOC.ChangeField=21		 THEN CAST(ISNULL(AC.TotalProvision,0) AS VARCHAR(MAX))
              WHEN MOC.ChangeField=24		 THEN TWO.StatusType
...
---------POSTMOC
 ...
 SELECT  C.CustomerID  
		...
		,CASE WHEN A.ChangeField=2		 THEN 'POS in Rs.'
              ...
              WHEN A.ChangeField=6		 THEN 'Fraud Flag'
