...
CREATE PROCEDURE [dbo].[ValidateExcel_Upload_07062022] 
	--@XmlData XML=''  
	--,@MenuID INT=NULL  
 --   ,@TimeKey int   
 --   ,@Result int=0 output    
...
BEGIN
  ...
  (  
   ...
   ,EntityId int identity(1,1)
	...
	PRINT @TimeKey
	  ...
	  DECLARE @DepartmentCode VARCHAR(50),@DepartmentAlt_Key int ,@UserRole int
	SELECT @DepartmentCode = DEP.DeptGroupCode,@DepartmentAlt_Key=DEP.DeptGroupId,@UserRole=UserRoleAlt_Key FROM DimUserInfo	INFO
  ...
  BEGIN
	 ...
	 SELECT 
				...
				--SELECT '#UserMasterUploadData', * FROM #UserMasterUploadData
	...
	inner join DimUserRole R ON R.RoleDescription=T.UserRole
	 ...
	 AND @UserRole <> 1
	  ...
	  INSERT INTO #OAOLMasterUploadData    
		...
		isnull(T.Designation,'') NOT IN  (select ParameterName from DimParameter  where
 ...
 --   FROM #TempSelectedBACIDsUserWise T  
	...
	Distinct  t.SrNo  
     -- ,RowNo  
      ,'Email Id' ColumnName  
      ,T.UserEmailId ErrorData  
      ,'Invalid Email id ,Email Id should end with @Emiratesnbd.com' ErrorType  
	  ,T.UserID
    FROM #UserMasterUploadData T  
	where T.UserEmailId not  like '%@Emiratesnbd.com' 
	AND ISNULL(T.UserEmailId,'') <> ''
...
-----User Mobile No
	...
	AND len(T.UserMobileNumber) < 10 or len(T.UserMobileNumber) > 10
