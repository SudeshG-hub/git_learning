...
----Select * 
from ENBD_TEMPDB.DBO.TempInvestmentIssuerDetail A
...
INNER JOIN ENBD_TEMPDB.DBO.TempInvestmentIssuerDetail AS T
...
AND NOT EXISTS (SELECT 1 FROM ENBD_TEMPDB.DBO.TempInvestmentIssuerDetail BB
    WHERE  AA.IssuerID=BB.IssuerID
    AND BB.EffectiveToTimeKey =49999
    )

INSERT INTO DBO.InvestmentIssuerDetail
     (	
       EntityKey
	  ,IssuerEntityId
	  ,[BranchCode]   
...
SELECT
	  ...
	  ,PanNo
	  ,NULL,NULL,NULL
		   FROM ENBD_TEMPDB.dbo.TempInvestmentIssuerDetail T Where ISNULL(T.IsChanged,'U') IN ('N','C')
