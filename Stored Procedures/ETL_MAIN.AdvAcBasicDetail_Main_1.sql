...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [ETL_MAIN].[AdvAcBasicDetail_Main_1]
...
INNER JOIN ENBD_TEMPDB.DBO.TempAdvAcBasicDetail AS T
...
AND NOT EXISTS (SELECT 1 FROM ENBD_TEMPDB.DBO.TempAdvAcBasicDetail BB
...
SELECT @Ac_Key=MAX(Ac_Key) FROM  ENBD_MISDB.[dbo].[AdvAcBasicDetail] 
IF @Ac_Key IS NULL  
BEGIN
SET @Ac_Key=0
END
...
SET TEMP.Ac_Key=ACCT.Ac_Key
 FROM ENBD_TEMPDB.DBO.[TempAdvAcBasicDetail] TEMP
INNER JOIN (SELECT CustomerAcId,(@Ac_Key + ROW_NUMBER()OVER(ORDER BY (SELECT 1))) Ac_Key
			FROM ENBD_TEMPDB.DBO.[TempAdvAcBasicDetail]
...
,ReferencePeriod
,CountryofExposure
FROM ENBD_TEMPDB.DBO.TempAdvAcBasicDetail T Where ISNULL(T.IsChanged,'U') IN ('N','C')
