...
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
CREATE PROC [dbo].[UserCreationInsert_New]
...
------------Added for Rejection Screen  29/06/2020   ----------
 ...
 SET @EffectiveFromTimeKey  = @TimeKey
	...
	IF @OperationFlag =1 AND @AuthMode ='Y' -- ADD
		...
		BEGIN
				SET @ApprovedBy	   = @CreatedModifyBy 
...
IF @DelStatus <>'DP' OR @AuthMode ='N'
	...
	,@IsSCD2 CHAR(1)='N'
		...
		IF ((@IsAvailable='N' OR @IsSCD2='Y') AND (@DelStatus <>'DP' or @DelStatus <>'D1'))
			BEGIN
				...
				PRINT 'Sachin20'
					...
					INSERT INTO DimUserInfo         
						...
						SELECT		
								...
								,@ApprovedBy
								,@DateApproved
								--UserMaster_ChangeFields
							FROM DimUserInfo_mod 
								WHERE (EffectiveFromTimeKey<=@TimeKey AND EffectiveToTimeKey>=@TimeKey)
									AND UserLoginID=@UserLoginID AND  AuthorisationStatus in('1A')
...
IF @ErrorHandle=0
	...
	IF @IsSCD2='Y' 
							BEGIN
								UPDATE DimUserInfo_mod 
								SET AuthorisationStatus =CASE WHEN @AUTHMODE='Y' THEN  'FM' ELSE NULL END
								WHERE (EffectiveFromTimeKey<=@TimeKey AND EffectiveToTimeKey>=@TimeKey) 
								AND UserLoginID=@UserLoginID
							END
