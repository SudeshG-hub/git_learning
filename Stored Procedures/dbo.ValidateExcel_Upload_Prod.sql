...
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
CREATE PROCEDURE [dbo].[ValidateExcel_Upload_Prod] 
	--@XmlData XML=''  
	--,@MenuID INT=NULL  
 --   ,@TimeKey int   
 --   ,@Result int=0 output    
 ...
 --   FROM #TempSelectedBACIDsUserWise T  
	...
	Distinct  t.SrNo  
     -- ,RowNo  
      ,'Email Id' ColumnName  
      ,T.UserEmailId ErrorData  
      ,'Invalid Email id ,Email Id should end with @ENBDbank.com' ErrorType  
	  ,T.UserID
    FROM #UserMasterUploadData T  
	where T.UserEmailId not  like '%@ENBDbank.com' 
	AND ISNULL(T.UserEmailId,'') <> ''
