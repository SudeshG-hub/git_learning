...
----Select * 
from ENBD_TEMPDB.DBO.TempAdvNFACFacDetail A
...
INNER JOIN ENBD_TEMPDB.DBO.TempAdvNFACFacDetail AS T
...
AND NOT EXISTS (SELECT 1 FROM ENBD_TEMPDB.DBO.TempAdvNFACFacDetail BB
...
SELECT @EntityKey=MAX(EntityKey) FROM  ENBD_MISDB.[dbo].[AdvAcNFFacDetail] 
...
SET TEMP.EntityKey=ACCT.EntityKey
 FROM ENBD_TEMPDB.DBO.[TempAdvAcNFFacDetail] TEMP
INNER JOIN (SELECT AccountEntityId,(@EntityKey + ROW_NUMBER()OVER(ORDER BY (SELECT 1))) EntityKey
			FROM ENBD_TEMPDB.DBO.[TempAdvAcNFFacDetail]
...
SELECT
	  ...
	  ,StockStmtDt
		   FROM ENBD_TEMPDB.DBO.TempAdvNFACFacDetail T Where ISNULL(T.IsChanged,'U') IN ('N','C')
