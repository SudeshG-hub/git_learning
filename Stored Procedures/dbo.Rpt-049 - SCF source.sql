...
create   PROCEDURE [dbo].[Rpt-049 - SCF source]
      @TimeKey AS INT,
	  @Cost    AS FLOAT,
	  @SelectReport AS INT
AS
...
SELECT 
DISTINCT
CONVERT(VARCHAR(20),Convert(date,@ProcessDate,105), 103)                  AS  [Report Date]
,SourceName                                      AS [Source System]
,B.UCIF_ID                                       AS UCIC
,A.RefCustomerID                                 AS CustomerID
,CustomerName                                    As [Borrower Name]
,B.Branchcode
,BranchName
...
,ISNULL(B.IntOverdue,0)/@Cost                  AS [Interest Dues]

,ISNULL(FIN.PenalOverdueinterest,0)/@Cost      AS [Penal Dues]
,ISNULL(B.OtherOverdue,0)/@Cost                AS [Other Dues]
...
WHERE @SelectReport=4 and B.FinalAssetClassAlt_key > 1
