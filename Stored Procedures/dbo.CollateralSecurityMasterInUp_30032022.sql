...
SET ANSI_NULLS ON
GO
Create PROC [dbo].[CollateralSecurityMasterInUp_30032022]
...
------------Added for Rejection Screen  29/06/2020   ----------
 ...
 SET @EffectiveFromTimeKey  = @TimeKey
	...
	PRINT 1
		-----CHECK DUPLICATE
		IF EXISTS(				                
					SELECT  1 FROM DimCollateralSubType WHERE CollateralSubTypeAltKey = @CollateralSubTypeAltKey AND ISNULL(AuthorisationStatus,'A')='A' and EffectiveFromTimeKey <=@TimeKey AND EffectiveToTimeKey >=@TimeKey
					UNION
	...
	IF @OperationFlag =1 AND @AuthMode ='Y' -- ADD
		BEGIN
					 ...
					 SET @CollateralSubTypeAltKey = (Select ISNULL(Max(CollateralSubTypeAltKey),0)+1 from ( 
													...
													UNION 
													Select CollateralSubTypeAltKey from DimCollateralSubType_Mod)A)
