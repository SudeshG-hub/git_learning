...
SET ANSI_NULLS ON
GO
CREATE PROC [dbo].[CollateralSecurityMasterInUp_Old]
						
						  @SecurityMappingAlt_Key Int=0
						 ,@SourceAlt_Key		  varchar(20)=''
						 ,@SrcSysSecurityCode	  Varchar(50)=''
						 ,@SrcSysSecurityName	  Varchar(50)=''  
						 ,@SecurityName			  Varchar(100)=''
						 ,@SecurityAlt_Key		  Int=0
						 ,@CollateralSecurityMaster_changeFields Varchar(500)=NULL 
...
------------Added for Rejection Screen  29/06/2020   ----------
		...
		DECLARE			@Uniq_EntryID			int	= 0
				...
				SET @ScreenName = 'CollateralSecurityMaster'
...
Insert into #Temp values(@SecurityAlt_Key,@SourceAlt_Key,@SecurityName)

Select A.Businesscolvalues1 as SourceAlt_Key,SecurityName,SecurityAlt_Key  into #final From (
...
---------------------------------
	...
	PRINT 1
		-----CHECK DUPLICATE
		IF EXISTS(				                
					SELECT  1 FROM DimCollateralSecurityMapping 
					WHERE  SourceAlt_Key in ( Select * from Split(@SourceAlt_Key,',')) 
					AND SrcSysSecurityCode=@SrcSysSecurityCode 
					AND ISNULL(AuthorisationStatus,'A')='A' 
					and EffectiveFromTimeKey <=@TimeKey AND EffectiveToTimeKey >=@TimeKey
					UNION
					SELECT  1 FROM DimCollateralSecurityMapping_Mod  
	...
	IF @OperationFlag =1 AND @AuthMode ='Y' -- ADD
		...
		-- DELETE WITHOUT MAKER CHECKER
						...
						UPDATE DimCollateralSecurityMapping SET
									ModifiedBy =@Modifiedby 
									,DateModified =@DateModified 
...
ELSE IF @OperationFlag=21 AND @AuthMode ='Y' 
		...
		IF EXISTS(SELECT 1 FROM DimCollateralSecurityMapping WHERE (EffectiveFromTimeKey<=@TimeKey AND EffectiveToTimeKey>=@Timekey) 
		                                      AND SecurityMappingAlt_Key =@SecurityMappingAlt_Key)
				BEGIN
					UPDATE DimCollateralSecurityMapping
...
--------------------------------
				...
				IF EXISTS(SELECT 1 FROM DimCollateralSecurityMapping WHERE (EffectiveFromTimeKey<=@TimeKey AND EffectiveToTimeKey>=@Timekey) AND SecurityMappingAlt_Key=@SecurityMappingAlt_Key)
				BEGIN
					UPDATE DimCollateralSecurityMapping
						SET AuthorisationStatus='A'
					WHERE (EffectiveFromTimeKey<=@TimeKey AND EffectiveToTimeKey>=@TimeKey)
	...
	---set parameters and UPDATE mod table in case maker checker enabled
			IF @AuthMode='Y'
				BEGIN
					...
					Print 'C'
					SELECT @ExEntityKey= MAX(EntityKey) FROM DimCollateralSecurityMapping_Mod 
						WHERE (EffectiveFromTimeKey<=@Timekey AND EffectiveToTimeKey >=@Timekey) 
							AND SecurityMappingAlt_Key=@SecurityMappingAlt_Key
							AND AuthorisationStatus IN('NP','MP','DP','RM','1A')	
					...
					SELECT @ExEntityKey= MIN(EntityKey) FROM DimCollateralSecurityMapping_Mod 
						WHERE (EffectiveFromTimeKey<=@Timekey AND EffectiveToTimeKey >=@Timekey) 
							AND SecurityMappingAlt_Key=@SecurityMappingAlt_Key
							AND AuthorisationStatus IN('NP','MP','DP','RM','1A')	
								...
								IF EXISTS(SELECT 1 FROM DimCollateralSecurityMapping WHERE (EffectiveFromTimeKey<=@TimeKey AND EffectiveToTimeKey>=@TimeKey) 
									...
									BEGIN
										...
										UPDATE DimCollateralSecurityMapping SET
												...
												,SrcSysSecurityCode			= @SrcSysSecurityCode	 
												,SrcSysSecurityName			= @SrcSysSecurityName	 
