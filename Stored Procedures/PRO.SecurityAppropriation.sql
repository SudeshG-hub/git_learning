...
UPDATE A set ApprRV= ((A.NETBALANCE/A.BALANCE)*A.SecurityValue)
--CASE WHEN  ((A.NETBALANCE/A.BALANCE)*A.SecurityValue)>A.NETBALANCE THEN A.NETBALANCE       
--ELSE ((A.NETBALANCE/A.BALANCE)*A.SecurityValue) END  
...
AND A.FlgSecured='D'     
AND isnull(SecurityValue,0)=0 AND A.FacilityType NOT in('LC','BG','NF')
...
AND D.FlgSecured='D'
AND isnull(SecurityValue,0)=0 AND D.FacilityType not  in('LC','BG','NF')
