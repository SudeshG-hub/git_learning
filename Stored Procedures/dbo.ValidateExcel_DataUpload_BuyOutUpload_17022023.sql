...
create PROCEDURE [dbo].[ValidateExcel_DataUpload_BuyOutUpload_17022023] 
@MenuID INT=1466,  
...
--								----),1,1,'')   
 ...
 ----------------------------------------------
 /*validations on ENBDAcNo*/
  
  UPDATE UploadBuyout
	SET  
        ErrorMessage=CASE WHEN ISNULL(ErrorMessage,'')='' THEN 'ENBDAcNo cannot be blank . Please check the values and upload again'     
						ELSE ErrorMessage+','+SPACE(1)+'ENBDAcNo cannot be blank . Please check the values and upload again'     END
		,ErrorinColumn=CASE WHEN ISNULL(ErrorinColumn,'')='' THEN 'ENBDAcNo' ELSE   ErrorinColumn +','+SPACE(1)+'ENBDAcNo' END   
		,Srnooferroneousrows=V.SlNo
								--STUFF((SELECT ','+SlNo 
								--FROM UploadBuyout A
								--WHERE A.SlNo IN(SELECT V.SlNo  FROM UploadBuyout V  
 ...
 WHERE ISNULL(ENBDAcNo,'')=''
  ...
  UPDATE UploadBuyout
	SET  
        ErrorMessage=CASE WHEN ISNULL(ErrorMessage,'')='' THEN 'Invalid ENBDAcNo.  Please check the values and upload again'     
						ELSE ErrorMessage+','+SPACE(1)+'Invalid ENBDAcNo.  Please check the values and upload again'     END
		,ErrorinColumn=CASE WHEN ISNULL(ErrorinColumn,'')='' THEN 'ENBDAcNo' ELSE   ErrorinColumn +','+SPACE(1)+'ENBDAcNo' END       
		,Srnooferroneousrows=V.SlNo
	--	STUFF((SELECT ','+SlNo 
	--							FROM UploadBuyout A
...
UPDATE UploadBuyout
	SET  
        ErrorMessage=CASE WHEN ISNULL(ErrorMessage,'')='' THEN  'You cannot perform Buyout, Record is pending for authorization for this Account ID. Kindly authorize or Reject the record through ‘Buyout Screen Upload– Authorization’ menu'     
						ELSE ErrorMessage+','+SPACE(1)+'You cannot perform Buyout, Record is pending for authorization for this Account ID. Kindly authorize or Reject the record through ‘Buyout Screen Upload– Authorization’ menu'     END
