...
INNER JOIN (SELECT CustomerAcID,TxnRefNo,TxnDate,TxnType,Particular,(@ENTITYKEY + ROW_NUMBER()OVER(ORDER BY (SELECT 1))) ENTITYKEY
			...
			AND TEMP.TxnDate=ACCT.TxnDate AND TEMP.TxnType=ACCT.TxnType AND ISNULL(TEMP.Particular,'AA')=ISNULL(ACCT.Particular,'AA')
			AND TEMP.TxnRefNo = ACCT.TxnRefNo

			--update ENBD_TEMPDB.DBO.TempACDailyTXNDetail set ENTITYKEY = 70597 where ENTITYKEY is NULL
...
--BEGIN
--Update ENBD_STGDB.dbo.PACKAGE_AUDIT SET ExecutionStatus = -1 where Tablename = 'AcDailyTxnDetail' and PackageName = 'TempToMainDB' 
