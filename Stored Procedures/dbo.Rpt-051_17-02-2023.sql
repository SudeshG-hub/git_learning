...
CREATE PROC [dbo].[Rpt-051_17-02-2023]
	@TimeKey  INT
	AS
...
INTO #PRE_MOC		
FROM #MOCUCIC_1  A
INNER JOIN  #InvestmentDerivative B ON  A.UCIC_ID=B.UCIC_ID
...
---------POSTMOC
 ...
 SELECT  A.UCIC_ID    
		...
		,CASE WHEN (A.ChangeFld=2 AND ScreenFlag='S')	OR (A.ChangeFld=3 AND ScreenFlag='U')		      THEN ISNULL(D.AssetClassName,'') 
...
WHERE (CASE WHEN (A.ChangeFld=2 AND ScreenFlag='S')	OR (A.ChangeFld=3 AND ScreenFlag='U')		      THEN ISNULL(D.AssetClassName,'') 
		    WHEN (A.ChangeFld=3 AND ScreenFlag='S')	OR (A.ChangeFld=4 AND ScreenFlag='U')		      THEN CONVERT(VARCHAR(20),C.NPA_Date,103)
