...
DECLARE @Code VARCHAR(20)  
 ...
 SET NOCOUNT ON;    
   ...
   SET @UserLocation=(SELECT CASE WHEN ISNULL(UserLocation,'')='' THEN 'HO' ELSE ISNULL(UserLocation,'') END  FROM DimUserInfo WHERE  
                     (DimUserInfo.EffectiveFromTimekey<=@TimeKey AND DimUserInfo.EffectiveToTimekey>=@TimeKey) and  UserLoginID =@UserLoginID)   
   SET @Code=(SELECT CASE WHEN ISNULL(UserLocation,'')='' THEN 'HO' ELSE ISNULL(UserLocation,'') END  FROM DimUserInfo WHERE   
                     (DimUserInfo.EffectiveFromTimekey<=@TimeKey AND DimUserInfo.EffectiveToTimekey>=@TimeKey) and UserLoginID =@UserLoginID )  
  ...
  DECLARE @DepartmentAlt_Key int ,@DepartmentCode VARCHAR(MAX)
	...
	INNER JOIN DimUserDeptGroup	DEP
		...
		PRINT '@DepartmentAlt_Key'+CAST(@DepartmentAlt_Key AS VARCHAR(20))
-------  
IF (@Tier = '4')  
     BEGIN  
   IF  @UserLocation = 'HO'  
...
IF  @UserLocation = 'ZO'  
   BEGIN  
	...
	SELECT ZoneName AS BranchName, 'ZO' AS BranchType FROM DIMZone 
	 ...
	 IF @UserRole_Key=1
		BEGIN
			SELECT UserRoleAlt_Key , RoleDescription FROM  DimUserRole ORDER BY 2
		END   
	ELSE IF @UserRole_Key=2
		BEGIN  
		   SELECT UserRoleAlt_Key , RoleDescription FROM  DimUserRole   
 ...
 END    
  ...
  --  AND EffectiveToTimeKey  > = @TimeKey
   ...
   SELECT	UserDeletionReasonAlt_Key AS Code
	...
	select ParameterAlt_Key as DesignationAlt_Key ,
			 ParameterName as DesignationName
			 ,'DimUserDesignation' as TableName
			 ,ParameterShortName as DesignationShortName
			 from DimParameter
			 where EffectiveFromTimeKey<=@Timekey And EffectiveToTimeKey>=@Timekey and
			  DimParameterName	= 'DimUserDesignation'
...
Select DeptGroupId as Code,DeptGroupCode as [Description],'DimUserDeptGroup' as TableName from DimUserDeptGroup
...
AND (
 ...
 --FROM dbo.Split(@ApplicableSolId,',')  
	...
	SELECT @DEPTALTKEY=DeptGroupId 
	FROM DimUserDeptGroup
	WHERE DeptGroupCode=@DeptCode
	AND EffectiveFromTimeKey<=@TimeKey AND EffectiveToTimeKey>=@TimeKey
	
select *, 'UserMaster' AS TableName  from MetaScreenFieldDetail where ScreenName = 'UserMaster'
	...
	--BEGIN
			PRINT 'CBO'
	--		IF OBJECT_ID ('TEMPDB..#TempDimDepartmentBACID') IS NOT NULL
