...
CREATE PROCEDURE [dbo].[PUIValidation_Prod]
...
,CASE WHEN C.value('./OriginalDCCO	[1]','VARCHAR(20)')='' THEN NULL ELSE C.value('./OriginalDCCO[1]','VARCHAR(20)') END AS OriginalEnvisagCompletionDt
,CASE WHEN C.value('./RevisedDCCO	[1]','VARCHAR(20)')='' THEN NULL ELSE C.value('./RevisedDCCO[1]','VARCHAR(20)') END AS RevisedCompletionDt
...
****************************************************************************************************************/
			...
			--SELECT 1
				...
				WHERE ISDATE(OriginalEnvisagCompletionDt)=1
				AND (CASE	WHEN SUBSTRING(RTRIM(LTRIM(OriginalEnvisagCompletionDt)),3,1)='-' 
								AND (LEN(RTRIM(LTRIM(OriginalEnvisagCompletionDt)))=9 OR LEN(RTRIM(LTRIM(OriginalEnvisagCompletionDt)))=11 )
								AND ISNUMERIC(SUBSTRING(RTRIM(LTRIM(OriginalEnvisagCompletionDt)),4,3))=0 
								AND  SUBSTRING(RTRIM(LTRIM(OriginalEnvisagCompletionDt)),7,1)='-' 
							THEN 1

							WHEN SUBSTRING(RTRIM(LTRIM(OriginalEnvisagCompletionDt)),3,1)='/'
							AND (LEN(RTRIM(LTRIM(OriginalEnvisagCompletionDt)))=8 OR LEN(RTRIM(LTRIM(OriginalEnvisagCompletionDt)))=10 )
							 AND  SUBSTRING(RTRIM(LTRIM(OriginalEnvisagCompletionDt)),6,1)='/' THEN 1
					END)=1
			)B 
...
print 'A'
	UPDATE A
				SET ERROR = 
								CASE	
										WHEN  (Convert(date,A.RevisedCompletionDt,103)<convert(date,A.OriginalEnvisagCompletionDt,103)) THEN 
...
****************************************************************************************************************/
		...
		SET ERROR = CASE	WHEN ISNULL(A.ProjectDelReason,'')=''	THEN 'Project Delay Reason should not be Empty'
							WHEN ISNULL(B.ParameterName,'')=''	THEN 'Invalid Project Delay Reason'
							ELSE ERROR
					END
		FROM #PUIData A
		LEFT OUTER JOIN DimParameter B
...
****************************************************************************************************************/
		...
		SET ERROR = CASE	WHEN ISNULL(A.StandardRestruct,'')=''	THEN 'StandardRestruct should not be Empty'
							WHEN ISNULL(B.ParameterName,'')=''	THEN 'Invalid StandardRestruct'
							ELSE ERROR
					END
		FROM #PUIData A
		LEFT OUTER JOIN DimParameter B
