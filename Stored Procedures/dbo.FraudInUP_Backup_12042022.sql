...
CREATE PROCEDURE [dbo].[FraudInUP_Backup_12042022]
...
BEGIN
	SET NOCOUNT ON;
		...
		SET DATEFORMAT DMY
			SET DATEFORMAT DMY	
    set @RFA_DateReportingByBank =case when ( @RFA_DateReportingByBank='' or @RFA_DateReportingByBank='01/01/1900' or @RFA_DateReportingByBank='1900/01/01') then NULL ELSE @RFA_DateReportingByBank END 
	set @RFA_OtherBankDate =case when ( @RFA_OtherBankDate='' or @RFA_OtherBankDate='01/01/1900' or @RFA_OtherBankDate='1900/01/01') then NULL 
	ELSE @RFA_OtherBankDate END 
	set @FraudOccuranceDate =case when ( @FraudOccuranceDate='' or @FraudOccuranceDate='01/01/1900' or @FraudOccuranceDate='1900/01/01') then NULL 
	ELSE @FraudOccuranceDate END
	set @FraudDeclarationDate =case when ( @FraudDeclarationDate='' or @FraudDeclarationDate='01/01/1900' or @FraudDeclarationDate='1900/01/01') then NULL 
 ...
 SET @EffectiveFromTimeKey  = @TimeKey
	...
	set  @AccountEntityID = (select AccountEntityID from curdat.advacbasicdetail where CustomerACID=@RefCustomerACID
	                           AND  EffectiveFromTimeKey <= @TimeKey AND EffectiveToTimeKey >= @TimeKey)
	
	set  @CustomerEntityID = (select CustomerEntityId from curdat.advacbasicdetail where CustomerACID=@RefCustomerACID
	                           AND  EffectiveFromTimeKey <= @TimeKey AND EffectiveToTimeKey >= @TimeKey)

	set  @RefCustomerid = (select RefCustomerId from curdat.advacbasicdetail where CustomerACID=@RefCustomerACID
	                           AND  EffectiveFromTimeKey <= @TimeKey AND EffectiveToTimeKey >= @TimeKey)
...
IF (select count(distinct AssetClassAtFraudAltKey) 
	...
	set  @AssetClassatFraudAltKey = (	select AssetClassAtFraudAltKey from Fraud_Details_Mod where AccountEntityId=@AccountEntityID
										AND  EffectiveFromTimeKey <= @TimeKey AND EffectiveToTimeKey >= @TimeKey And ISNULL(AuthorisationStatus,'A') in ('NP','MP','1A','A')
									)

	
	set  @NPADtatFraud =		(		select NPA_DateAtFraud from Fraud_Details_Mod where AccountEntityId=@AccountEntityID
	                           AND  EffectiveFromTimeKey <= @TimeKey AND EffectiveToTimeKey >= @TimeKey And ISNULL(AuthorisationStatus,'A') in ('NP','MP','1A','A')
								)
								END
								ELSE
								BEGIN
								set  @AssetClassatFraudAltKey = (	select	Cust_AssetClassAlt_Key 
																	from	dbo.AdvCustNPADetail 
