...
CREATE PROCEDURE [dbo].[Rpt-029A_09-12-20222]	
    @TimeKey AS INT,
	@Cost AS FLOAT,
	@SelectReport AS INT
...
0                                                 AS SecurityUsedRV,
ISNULL(IFD.Interest_DividendDueAmount,0)/@Cost    AS InterestInSuspenseAmount,
...
(ISNULL(IFD.TotalProvison,0)/NULLIF(IFD.BookValueINR,0))*100                                                  AS [Total Provision %]
,NULL AS NPAReason
FROM dbo.InvestmentBasicDetail IBD
INNER JOIN DBO.InvestmentFinancialdetail     IFD			ON  IFD.InvEntityId=IBD.InvEntityId 
...
ISNULL(POS,0)/@Cost                               AS PrincipalOutstanding,
ISNULL(MTMIncomeAmt,0)/@Cost                      AS NetBalance,
0                                                 AS SecurityValue,
0                                                 AS SecurityValueappropriated,
''                                                AS SecurityType,
''                                                AS ValuationDate,
...
0                                                 AS UnsecuredOutstanding,
0                                                 AS SecurityUsedRV,
ISNULL(Derivative.MTMIncomeAmt,0)/@Cost           AS InterestInSuspenseAmount,
0                                                 AS Totalincomesuspended,
CASE WHEN ISNULL(DA.AssetClassName,'')  <>'STANDARD'
...
0                                                 AS ProvisionSecured,
0                                                 AS ProvisionUnsecured,
(ISNULL(Derivative.TotalProvison,0)/NULLIF(Derivative.MTMIncomeAmt,0))*100                                                 AS [Total Provision %]
,NULL AS NPAReason
FROM CURDAT.DerivativeDetail Derivative
