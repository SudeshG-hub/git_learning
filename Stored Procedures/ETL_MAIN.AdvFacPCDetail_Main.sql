...
----Select * 
from ENBD_TEMPDB.DBO.TempAdvFacPCDetail A
...
INNER JOIN ENBD_TEMPDB.DBO.TempAdvFacPCDetail AS T
...
AND NOT EXISTS (SELECT 1 FROM ENBD_TEMPDB.DBO.TempAdvFacPCDetail BB
...
SELECT @EntityKey=MAX(EntityKey) FROM  ENBD_MISDB.[dbo].[AdvFacPCDetail] 
...
SELECT
      ...
      ,[LcAmtInCurrenc]
	  --,PcEntityId
		   FROM ENBD_TEMPDB.dbo.TempAdvFacPCDetail T Where ISNULL(T.IsChanged,'U') IN ('N','C')
