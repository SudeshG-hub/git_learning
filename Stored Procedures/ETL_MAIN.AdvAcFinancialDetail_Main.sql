...
----Select * 
from ENBD_TEMPDB.DBO.TempAdvAcFinancialDetail A
...
INNER JOIN ENBD_TEMPDB.DBO.TempAdvAcFinancialDetail AS T
...
AND NOT EXISTS (SELECT 1 FROM ENBD_TEMPDB.DBO.TempAdvAcFinancialDetail BB
...
SELECT @EntityKey=MAX(EntityKey) FROM  ENBD_MISDB.[dbo].[AdvAcFinancialDetail] 
...
SET TEMP.EntityKey=ACCT.EntityKey
 FROM ENBD_TEMPDB.DBO.[TempAdvAcFinancialDetail] TEMP
INNER JOIN (SELECT AccountEntityId,(@EntityKey + ROW_NUMBER()OVER(ORDER BY (SELECT 1))) EntityKey
			FROM ENBD_TEMPDB.DBO.[TempAdvAcFinancialDetail]
...
,NULL AccountBlkCode2
	...
	FROM ENBD_TEMPDB.DBO.TempAdvAcFinancialDetail T Where ISNULL(T.IsChanged,'U') IN ('N','C')
