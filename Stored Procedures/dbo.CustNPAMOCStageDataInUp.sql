...
IF (@OperationFlag=16)----AUTHORIZE
	...
	BEGIN
				...
				IF (@UserLoginID =(Select distinct CreatedBy from CustomerLevelMOC_MOD 
...
--	END
	...
	BEGIN
	          ...
	          IF (@UserLoginID =(Select distinct CreatedBy from CustomerLevelMOC_MOD where  UploadId=@UniqueUploadID Group By CreatedBy))
	          BEGIN
								SET @Result=-1
...
IF (@OperationFlag=21)----REJECT
	...
	BEGIN
	    ...
	    IF @UserLoginID= (select UserLoginID from DimUserInfo where  IsChecker2='N' and UserLoginID=@UserLoginID and EffectiveToTimeKey=49999)
			...
			ELSE BEGIN
				IF (@UserLoginID =(Select distinct CreatedBy from CustomerLevelMOC_MOD where AuthorisationStatus IN ('1A') and UploadId=@UniqueUploadID 
				and EffectiveToTimeKey = 49999 and CreatedBy = @UserLoginID
				                     --AND CreatedBy in (select createdby from DimUserInfo where  IsChecker2='N')
								   Group By CreatedBy))
				...
				ELSE
				BEGIN
					IF (@UserLoginID =(Select distinct  ApprovedBy from CustomerLevelMOC_MOD where AuthorisationStatus IN ('1A') and UploadId=@UniqueUploadID 
					and EffectiveToTimeKey = 49999 and ApprovedBy = @UserLoginID
				                     --AND CreatedBy in (select createdby from DimUserInfo where  IsChecker2='N')
								   Group By ApprovedBy))
