...
CREATE PROCEDURE [PRO].[UpdateNetBalance_AccountWise_A]        
 @TimeKey int 
 with recompile       
AS       
 BEGIN       
...
UPDATE A         
SET  
		NetBalance = ISNULL(PrincOutStd,0)
	FROM PRO.AccountCal A       
INNER JOIN DimAssetClass B ON B.AssetClassAlt_Key=isnull(A.FinalAssetClassAlt_Key,1)
     AND (B.EffectiveFromTimeKey<=@TimeKey AND B.EffectiveToTimeKey>=@TimeKey)
WHERE  B.ASSETCLASSGROUP='NPA' AND A.FacilityType NOT IN ('LC','BG','NF')


UPDATE A         
SET  
		NetBalance =  ISNULL(PrincOutStd,0)-isnull(Amount,0)
	FROM PRO.AccountCal A       
	INNER JOIN DimAssetClass B ON B.AssetClassAlt_Key=isnull(A.FinalAssetClassAlt_Key,1)
		 AND (B.EffectiveFromTimeKey<=@TimeKey AND B.EffectiveToTimeKey>=@TimeKey)
	...
	WHERE  ISNULL(A.AddlProvisionPer,0)<>0
		...
		----- Start  Non Funded NET Amount Work----- 
			...
			WHERE ISNULL(A.Balance,0)>0
				...
				AND (B.EffectiveFromTimeKey<=@TimeKey AND B.EffectiveToTimeKey>=@TimeKey)
				AND A.FacilityType IN ('LC','BG','NF')
...
/* SECURITISATIONS */
		...
		SET  NetBalance = NetBalance-ISNULL(B.ExposureAmount,0)
		FROM PRO.AccountCal A  
			INNER JOIN SecuritizedACFlaggingDetail B
				ON B.EffectiveFromTimeKey<=@TimeKey AND B.EffectiveToTimeKey>=@TimeKey
				AND A.CustomerAcID=B.AccountID
		WHERE poolType='PTC'
...
UPDATE A         
		SET  
		NetBalance = 0 
		FROM PRO.AccountCal A  where ISNULL(NetBalance,0)<0
SELECT * FROM SecuritizedACFlaggingDetail
...
update pro.ACCOUNTCAL set UnserviedInt=0  where ISNULL(UnserviedInt,0)<0
