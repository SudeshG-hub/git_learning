...
CREATE PROC [dbo].[CustomerLevelNPAMOCSearchList_05022022]
...
SET NOCOUNT ON;
 ...
 -- SET @Timekey =(Select LastMonthDateKey from SysDayMatrix where Timekey=@Timekey) 
	...
	SET @Timekey =(Select Timekey from SysDataMatrix Where MOC_Initialised='Y' AND ISNULL(MOC_Frozen,'N')='N') 
	 ...
	 SET @SourceSystem = (select distinct SourceName from [Pro].CustomerCal_Hist A  INNER JOIN DIMSOURCEDB B ON A.SourceAlt_key = B.SourceAlt_Key where RefCustomerId = @CustomerID ) 
...
SELECT  Distinct 
	...
	@MocReason=MocReason,@MOCreasonAltkey = b.ParameterAlt_Key,@MOCSourceAltkey=MOCSourceAltkey,@CreatedBy=CreatedBy,
...
case when A.Cust_AssetClassAlt_Key is null then 1 else A.Cust_AssetClassAlt_Key end AssetClassAlt_Key,A.NPADt,V.security_value  SecurityValue,B.UCIF_ID as UCICID, 0 as AdditionalProvision	
...
and           b.EffectiveFromTimeKey<=@timekey and b.EffectiveFromTimeKey>=@timekey 
...
where         a.EffectiveFromTimeKey<=@timekey and a.EffectiveFromTimeKey>=@timekey 
...
DROP TABLE IF EXISTS #CUST_POSTMOC
 ...
 --   FROM MOC_ChangeDetails A
	...
	@DateApproved as DateApproved,@ApprovedByFirstLevel as ApprovedByFirstLevel,
	@DateApprovedFirstLevel as DateApprovedFirstLevel,@MOC_ExpireDate MOC_ExpireDate,
	@MOC_TYPEFLAG MOCType_Flag,A.MOCType as	MOCTypeAlt_Key,B.SourceSystemAlt_Key as SourceSystemAlt_Key1
	,Convert(Varchar(50),'') SourceSystemAlt_Key,@MOCreasonAltkey MOCReasonAlt_Key
    FROM CustomerLevelMOC_Mod A
	INNER JOIN  CURDAT.CustomerBasicDetail B
	ON          A.CustomerEntityID=B.CustomerEntityId
	AND         B.EffectiveFromTimeKey<=@TimeKey and B.EffectiveToTimeKey>=@TimeKey 
...
--
	...
	,@ApprovedByFirstLevel as ApprovedByFirstLevel,@DateApprovedFirstLevel as DateApprovedFirstLevel,
	@MOC_ExpireDate MOC_ExpireDate,@MOC_TYPEFLAG MOCType_Flag,case when MOCTYPE = 'Auto' then 1 else 2 end  MOCTypeAlt_Key,B.SourceSystemAlt_Key as SourceSystemAlt_Key1,Convert(Varchar(50),'') SourceSystemAlt_Key,@MOCreasonAltkey MOCReasonAlt_Key
	 FROM MOC_ChangeDetails A
...
END
	...
	PRINT 'Sudesh'
	Update A
	SET A.SourceSystemAlt_Key=B.SourceName
	From #CUST_POSTMOC A
