...
SET ANSI_NULLS ON
GO
create PROC [dbo].[SMAClassMaster_InUp_16052022]
...
Insert into #Temp values(@AssetClassAlt_Key--,@SourceAlt_Key
,@AssetClassName)

Select --A.Businesscolvalues1 as SourceAlt_Key,
AssetClassName,AssetClassAlt_Key  into #final From (
...
*/
	...
	*/			
	...
	PRINT 1
		-----CHECK DUPLICATE
		IF EXISTS(				                
					SELECT  1 FROM DimSMAClassMaster 
					WHERE  --SourceAlt_Key in ( Select * from Split(@SourceAlt_Key,',')) 
					 SrcSysClassCode=@SrcSysAssetClassCode 
			...
			BEGIN
			   PRINT 3
					 SET @AssetClassMappingAlt_Key = (Select ISNULL(Max(AssetClassMappingAlt_Key),0)+1 from 
												(Select AssetClassMappingAlt_Key from DimSMAClassMaster
												 UNION 
												 Select AssetClassMappingAlt_Key from DimSMAClassMaster_Mod
												)A)
...
ELSE IF @OperationFlag=21 AND @AuthMode ='Y' 
		...
		IF EXISTS(SELECT 1 FROM DimSMAClassMaster WHERE (EffectiveFromTimeKey<=@TimeKey AND EffectiveToTimeKey>=@Timekey) 
		                                  AND AssetClassMappingAlt_Key =@AssetClassMappingAlt_Key)
				BEGIN
					UPDATE DimSMAClassMaster
...
--------------------------------
				...
				IF EXISTS(SELECT 1 FROM DimSMAClassMaster WHERE (EffectiveFromTimeKey<=@TimeKey AND EffectiveToTimeKey>=@Timekey) AND AssetClassMappingAlt_Key=@AssetClassMappingAlt_Key)
				BEGIN
					UPDATE DimSMAClassMaster
						SET AuthorisationStatus='A'
					WHERE (EffectiveFromTimeKey<=@TimeKey AND EffectiveToTimeKey>=@TimeKey)
	...
	---set parameters and UPDATE mod table in case maker checker enabled
			IF @AuthMode='Y'
				BEGIN
					...
					Print 'C'
					SELECT @ExEntityKey= MAX(EntityKey) FROM DimSMAClassMaster_Mod 
						WHERE (EffectiveFromTimeKey<=@Timekey AND EffectiveToTimeKey >=@Timekey) 
							AND AssetClassMappingAlt_Key=@AssetClassMappingAlt_Key
							AND AuthorisationStatus IN('NP','MP','DP','RM','1A')	
					...
					SELECT @ExEntityKey= MIN(EntityKey) FROM DimSMAClassMaster_Mod 
						WHERE (EffectiveFromTimeKey<=@Timekey AND EffectiveToTimeKey >=@Timekey) 
							AND AssetClassMappingAlt_Key=@AssetClassMappingAlt_Key
							AND AuthorisationStatus IN('NP','MP','DP','RM','1A')	
...
--END
	...
	SET @Parameter2 = (select STUFF((	SELECT Distinct ',' +ChangeFields
											from DimSMAClassMaster_Mod where  AssetClassMappingAlt_Key=@AssetClassMappingAlt_Key
