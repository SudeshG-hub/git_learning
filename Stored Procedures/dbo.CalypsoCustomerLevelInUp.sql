...
BEGIN  
 ...
 IF @Result = -1
	...
	set @NPADate = case when (@NPADate='' or  @NPADate='01/01/1900') then null else @NPADate end
 				...
 				DECLARE @Parameter1 varchar(max) = (select 
				'UCICID|' + ISNULL(@UCICID,' ') + '}'
				+ 'CustomerName|' + isnull(@CustomerName,' ')+ '}'+ 
				'AssetClassAlt_Key_Pos|'+convert(varchar,isnull(@AssetClassAlt_Key,''))+ '}'+ 
				'NPADate_Pos|'+isnull(@NPADate,'')+ '}'+ 'SecurityValue_Pos|'+isnull(@SecurityValue,'')+ '}'+ 
				'AdditionalProvision_Pos|'+convert(varchar,isnull(@AdditionalProvision,'0'))+ '} '+
				'MOCTypeAlt_Key|'+convert(varchar,isnull(@MocTypeAlt_Key,''))+ '}'+ 
				'MOCReason|'+isnull(@MOCReason,'')+ '}'+ 
