...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[ValidateExcel_DataUpload_Prov_Category_06082022]  
@MenuID INT=10,  
...
----------------------------------
  ...
  UPDATE UploadProCategory						
	SET  					
        ErrorMessage=CASE WHEN ISNULL(ErrorMessage,'')='' THEN 'Record for ACID  is pending for authorization in ‘Upload ID’ '+ Convert(Varchar(10),B.UploadId) +' kindly remove the record and upload again '     						
						ELSE ErrorMessage+','+SPACE(1)+'Record for ACID  is pending for authorization in ‘Upload ID’ '+ Convert(Varchar(10),B.UploadId) +' kindly remove the record and upload again '     END
		,ErrorinColumn=CASE WHEN ISNULL(ErrorinColumn,'')='' THEN 'ACID ' ELSE   ErrorinColumn +','+SPACE(1)+'ACID ' END       				
		,Srnooferroneousrows=V.SlNo				
    FROM UploadProCategory V  						
   LEFT Join AcCatUploadHistory_Mod B ON V.ACID=B.ACID			
...
--select *
 ...
 WHERE (CASE WHEN ISNUMERIC(V.CategoryID) = 1 THEN V.CategoryID else 0 END)  not in (select BankCategoryID  from DimProvision_SegStd A where A.BankCategoryID=V.CategoryID And A.EffectiveToTimeKey=49999)
...
--select *
 ...
 Where EXISTS(				                
					SELECT  1 FROM AcCatUploadHistory A WHERE A.CategoryID=V.CategoryID And A.ACID=V.ACID And A.EffectiveToTimeKey=49999 AND ISNULL(A.AuthorisationStatus,'A')='A' 
					UNION
					SELECT  1 FROM AcCatUploadHistory_Mod B WHERE B.CategoryID=V.CategoryID And B.ACID=V.ACID And B.EffectiveToTimeKey=49999 
