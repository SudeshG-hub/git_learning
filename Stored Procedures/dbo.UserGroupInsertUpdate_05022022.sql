...
CREATE PROCEDURE [dbo].[UserGroupInsertUpdate_05022022] -- -- select * from DimUserDeptGroup_Mod
 --@EntityKey int,        
 @DeptGroupId int = 1,        
...
DECLARE @AuthorisationStatus CHAR(2)=NULL			
   ...
   --          WHERE CONVERT(VARCHAR(10), Date, 103) = CONVERT(VARCHAR(10), GETDATE(), 103)
   --      );
   		Set @EffectiveFromTimeKey=(select CAST(B.timekey as int)from SysDataMatrix A
			Inner Join SysDayMatrix B ON A.TimeKey=B.TimeKey
			 where A.CurrentStatus='C')
