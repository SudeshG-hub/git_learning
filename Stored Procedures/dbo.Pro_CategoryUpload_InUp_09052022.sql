...
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
CREATE PROCEDURE  [dbo].[Pro_CategoryUpload_InUp_09052022]
	@Timekey INT,
...
*/
	...
	Insert Into #categorydetails_Upload(ACID,FieldName)
	 Select ACID, 'SlNo' FieldName from categorydetails_stg Where isnull(SlNo,'')<>'' 
	UNION ALL
	Select ACID, 'CustomerID' FieldName from categorydetails_stg Where isnull(CustomerID,'')<>'' 
	UNION ALL
	Select ACID, 'CategoryID' FieldName from categorydetails_stg Where isnull(CategoryID,'')<>''
	UNION ALL
	...
	Where A.MenuId=@MenuId And A.IsVisible='Y'
		...
		print 'nanda4'
				 ...
				 IF OBJECT_ID('TEMPDB..#NEWTRANCHE')  IS NOT NULL
					...
					INNER JOIN #NEWTRANCHE B ON A.ACID=B.ACID
...
END
	...
	BEGIN
		...
		IF (@UserLoginID =(Select CreatedBy from AcCatUploadHistory_MOD where AuthorisationStatus IN ('NP','MP') and UploadId=@UniqueUploadID
				...
				BEGIN
								...
								RETURN @Result
								--SELECT * FROM AcCatUploadHistory_MOD
				END
	...
	BEGIN
		  ...
		  IF @UserLoginID= (select UserLoginID from DimUserInfo where  IsChecker2='N' and UserLoginID=@UserLoginID )
			   BEGIN
				...
				IF (@UserLoginID =(Select CreatedBy from AcCatUploadHistory_MOD where AuthorisationStatus IN ('1A') and UploadId=@UniqueUploadID 
		...
		ELSE 
	BEGIN
		IF (@UserLoginID =(Select ApprovedByFirstLevel from AcCatUploadHistory_MOD where  ApprovedByFirstLevel=@UserLoginID and EffectiveToTimeKey = 49999 
...
*/
	...
	BEGIN
	  ...
	  IF @UserLoginID= (select  UserLoginID from DimUserInfo where  IsChecker2='N' and UserLoginID=@UserLoginID 
		...
		IF (@UserLoginID =(Select CreatedBy from AcCatUploadHistory_MOD where  UploadId=@UniqueUploadID  and  AuthorisationStatus IN ('1A') and EffectiveToTimeKey=49999 Group By CreatedBy))
			BEGIN
								SET @Result=-1
			...
			ELSE 
	BEGIN
		IF (@UserLoginID =(Select ApprovedByFirstLevel from AcCatUploadHistory_MOD where  ApprovedByFirstLevel=@UserLoginID   and  AuthorisationStatus IN ('1A') and EffectiveToTimeKey=49999
			                      Group By ApprovedByFirstLevel))
	          BEGIN
								SET @Result=-1
...
--------------------------------------------------------------------
	...
	BEGIN
		 ...
		 IF (@UserLoginID =(Select CreatedBy from AcCatUploadHistory_MOD where  UploadId=@UniqueUploadID  and  AuthorisationStatus IN ('NP','MP') and EffectiveToTimeKey=49999 Group By CreatedBy))
	          BEGIN
								SET @Result=-1
