...
from ENBD_TEMPDB.DBO.TempAdvAcOtherDetail A
...
INNER JOIN ENBD_TEMPDB.DBO.TempAdvAcOtherDetail AS T
...
AND NOT EXISTS (SELECT 1 FROM ENBD_TEMPDB.DBO.TempAdvAcOtherDetail BB
...
SELECT @EntityKey=MAX(EntityKey) FROM  ENBD_MISDB.[dbo].[AdvAcOtherDetail] 
...
SET TEMP.EntityKey=ACCT.EntityKey
 FROM ENBD_TEMPDB.DBO.[TempAdvAcOtherDetail] TEMP
INNER JOIN (SELECT AccountEntityId,(@EntityKey + ROW_NUMBER()OVER(ORDER BY (SELECT 1))) EntityKey
			FROM ENBD_TEMPDB.DBO.[TempAdvAcOtherDetail]
...
SELECT 
     ...
     FROM ENBD_TEMPDB.DBO.TempAdvAcOtherDetail T Where ISNULL(T.IsChanged,'U') IN ('N','C')
