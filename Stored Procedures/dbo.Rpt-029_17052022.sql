...
CREATE PROCEDURE [dbo].[Rpt-029_17052022]
      @TimeKey AS INT,
	  @Cost    AS FLOAT
AS
...
WHERE B.EffectiveFromTimeKey <= @LastQtrDateKey AND B.EffectiveToTimeKey >=@LastQtrDateKey
      AND A.EffectiveFromTimeKey <= @LastQtrDateKey AND A.EffectiveToTimeKey >=@LastQtrDateKey)Y 	ON B.AccountEntityId = Y.AccountEntityId
