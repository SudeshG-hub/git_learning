...
CREATE PROC [dbo].[CalypsoAccountLevelQuickSearchList_21092022]
 ...
 -- SET @Timekey =(Select LastMonthDateKey from SysDayMatrix where Timekey=@Timekey) 
   ...
   DECLARE @CustomerEntityID INT=0
	...
	SET @ExtDate=(Select Distinct ExtDate from SysDataMatrix Where ISNULL(MOC_Initialised,'N')='Y' AND ISNULL(MOC_Frozen,'N')='N') 
		   ...
		   SET  @CustomerEntityID =(SELECT IssuerEntityId  
...
--Select '#TEmp', * from #TEmp
	 ...
	 IF OBJECT_ID('TempDB..#tmp2') IS NOT NULL DROP Table #tmp2
		...
		select distinct A.AccountID as ACID,
				 ...
				 INNER JOIN curdat.DerivativeDetail  T  ON A.AccountEntityID=T.DerivativeRefNo AND                                                   
				 ...
				 from CalypsoAccountLevelMOC_Mod A
				   --     inner join Pro.accountcal_Hist C ON C.CustomerAcID=A.AccountID
						 ...
						 INNER JOIN curdat.DerivativeDetail B
						                 ...
						                 ON A.AccountEntityID=B.DerivativeRefNo
						                 AND B.EffectiveFromTimeKey <= @Timekey
						                 AND B.EffectiveToTimeKey >= @Timekey
		...
		select * from (
				...
				from CalypsoAccountlevelmoc_mod A
					 ...
					 INNER JOIN Curdat.DerivativeDetail C
						                 ...
						                 ON A.AccountEntityID=C.DerivativeEntityID
						                 AND C.EffectiveFromTimeKey <= @Timekey
						                 AND C.EffectiveToTimeKey >= @Timekey
...
ELSE
  BEGIN
	   ...
	   SELECT invEntityid  FROM  InvestmentBasicDetail A
		...
		Begin
		   IF EXISTS (SELECT 1 FROM CalypsoInvMOC_ChangeDetails WHERE AccountEntityID =@AccountEntityID 
		                                               AND EffectiveFromTimeKey<=@Timekey
					                    AND EffectiveToTimeKey >= @Timekey  AND AuthorisationStatus='A' 
										UNION SELECT 1 FROM CalypsoDervMOC_ChangeDetails WHERE AccountEntityID =@AccountEntityID 
		                                               AND EffectiveFromTimeKey<=@Timekey
					                    AND EffectiveToTimeKey >= @Timekey  AND AuthorisationStatus='A')
...
END	
 ...
 ELSE IF EXISTS (SELECT 1 FROM CalypsoAccountLevelMOC_Mod WHERE AccountEntityID =@AccountEntityID  AND EffectiveFromTimeKey<=@Timekey
