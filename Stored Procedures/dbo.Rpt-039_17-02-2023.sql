...
CREATE PROC [dbo].[Rpt-039_17-02-2023]
	@TimeKey  INT,
	@SelectReport AS INT
	AS 
...
,ISNULL(ProvPerSecured,0)	                                               AS ProvPerSecured
,ISNULL(ProvPerUnSecured,0)                                                AS ProvPerUnSecured
,ISNULL(A.ProvDFV,0)                                                       AS ProvDFV
,ISNULL(AddlProvision,0)                                                   AS AddlProvision
,ISNULL(FinalProvisionPer,0)                                               AS FinalProvisionPer
,ISNULL(A.WriteOffAmount,0)                                                AS WriteOffAmount
,ISNULL(A.NetBalance,0)                                                    AS NetBalance
,A.FlgDeg
,A.FlgUpg
,CASE WHEN DAC.AssetClassShortNameEnum='SUB'
	  THEN CONVERT(VARCHAR(20),A.FinalNpaDt,103) 
	  ELSE ''				
	  END                                                             AS SubSTD
,CASE WHEN DAC.AssetClassShortNameEnum='DB1'
	  THEN CONVERT(VARCHAR(20),DATEADD(YYYY,1,A.FinalNpaDt),103) 
	  ELSE ''				
	  END                                                             AS DB1
,CASE WHEN DAC.AssetClassShortNameEnum='DB2'
	  THEN CONVERT(VARCHAR(20),DATEADD(YYYY,2,A.FinalNpaDt),103) 
	  ELSE ''				
	  END										                      AS DB2

,CASE WHEN DAC.AssetClassShortNameEnum='DB3'
	  THEN CONVERT(VARCHAR(20),DATEADD(YYYY,4,A.FinalNpaDt),103) 
	  ELSE ''				
	  END										                      AS DB3

,CONVERT(VARCHAR(20),ACND.LosDt,103)                                  AS LossDt
,ISNULL(FIN.UnAppliedIntAmount,0)			                          AS InterestInSuspenseAmount
