...
CREATE PROCEDURE [dbo].[Rpt-029_23-05-2022]
      @TimeKey AS INT,
	  @Cost    AS FLOAT
AS
...
,CASE WHEN A2.AssetClassName ='STANDARD' AND Y.AssetClassName <>'STANDARD' THEN
...
,CASE WHEN A2.AssetClassName ='STANDARD' AND Y.AssetClassName <>'STANDARD' THEN
        (CASE WHEN ((ISNULL(B.NetBalance,0)-ISNULL(B.TotalProvision,0)) - ISNULL(Y.NetNPA1,0)) < 0 
       THEN 0 
	   ELSE ((ISNULL(B.NetBalance,0)-ISNULL(B.TotalProvision,0)) - ISNULL(Y.NetNPA1,0)) 
	   END)/@Cost                                                     
...
WHERE B.EffectiveFromTimeKey <= @LastQtrDateKey AND B.EffectiveToTimeKey >=@LastQtrDateKey
      AND A.EffectiveFromTimeKey <= @LastQtrDateKey AND A.EffectiveToTimeKey >=@LastQtrDateKey)Y 
