...
-- =============================================
CREATE PROCEDURE [dbo].[CustomerDetailSelect_12072022]
...
BEGIN
	...
	CREATE TABLE #CustomerDetailSelect
				...
				,AssetSubClass					Varchar(30)
				,SecurityValue					Decimal(18,2)
				)
				...
				IF @CustBasic='N' OR @OperationFlag<>16	-- FROM MAIN TABLE
					...
					PRINT 1
						...
						--ServProvider,NonCustType
												...
												,PC.CustMoveDescription as AssetSubClass
												,SA.CurrentValue as SecurityValue
						...
						LEFT JOIN Pro.CustomerCal_Hist PC ON PC.CustomerEntityID=C.CustomerEntityId 
						AND PC.EffectiveFromTimeKey<=@TimeKey and PC.EffectiveToTimeKey>=@TimeKey 
						LEFT JOIN (Select A.RefCustomerId,SUM(ISNULL(B.CurrentValue,0))CurrentValue from CurDat.AdvSecurityDetail A
									Inner Join CurDat.AdvSecurityValueDetail B ON A.SecurityEntityID=B.SecurityEntityID
									And B.EffectiveFromTimeKey<=@TimeKey and B.EffectiveToTimeKey>=@TimeKey
									where A.EffectiveFromTimeKey<=@TimeKey and A.EffectiveToTimeKey>=@TimeKey Group by A.RefCustomerId ) SA ON SA.RefCustomerId=C.CustomerId
