...
----Select * 
from ENBD_TEMPDB.DBO.TempAdvNFAcFinancialDetail A
...
INNER JOIN ENBD_TEMPDB.DBO.TempAdvNFAcFinancialDetail AS T
...
AND NOT EXISTS (SELECT 1 FROM ENBD_TEMPDB.DBO.TempAdvNFAcFinancialDetail BB
...
SELECT @EntityKey=MAX(EntityKey) FROM  ENBD_MISDB.[dbo].[AdvNFAcFinancialDetail] 
...
SET TEMP.EntityKey=ACCT.EntityKey
 FROM ENBD_TEMPDB.DBO.[TempAdvNFAcFinancialDetail] TEMP
INNER JOIN (SELECT AccountEntityId,(@EntityKey + ROW_NUMBER()OVER(ORDER BY (SELECT 1))) EntityKey
			FROM ENBD_TEMPDB.DBO.[TempAdvNFAcFinancialDetail]
...
,Ac_ReviewAuthLevelAlt_Key
	...
	FROM ENBD_TEMPDB.DBO.TempAdvNFAcFinancialDetail T Where ISNULL(T.IsChanged,'U') IN ('N','C')
