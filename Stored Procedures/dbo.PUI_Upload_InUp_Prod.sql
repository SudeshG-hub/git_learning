...
SET ANSI_NULLS ON
GO
CREATE PROC [dbo].[PUI_Upload_InUp_Prod]
...
--------------------------------------------------------------------
												...
												INSERT INTO advacprojectdetail
													...
													SELECT
														...
														,convert (date,RevisedCompletionDt,103)RevisedCompletionDt
														,convert (date,ActualCompletionDt,103) ActualCompletionDt
														,PC.ParameterAlt_Key
														...
														Inner Join (Select ParameterAlt_Key,ParameterName from DimParameter where DimparameterName='ProdectDelReson'  
														AND EffectiveFromTimeKey<=@TimeKey And EffectiveToTimeKey>=@TimeKey) PDR
														ON PDR.ParameterName=S.ProjectDelReason 
														Inner Join (Select ParameterAlt_Key,ParameterName from DimParameter where DimparameterName='DimYesNo'  
...
END
	...
	IF (@OperationFlag IN(1,2,3,16,20,17,21,18 )AND @AuthMode ='Y')
		...
		PRINT 5
				...
				END
					IF @OperationFlag in (17,21) and @Remark is null
						BEGIN 
									...
									INSERT INTO  AdvAcProjectDetail_Upload_Mod
										...
										SELECT
											...
											,Convert(Date,RevisedCompletionDt,103) as RevisedCompletionDt
											,Convert(Date,ActualCompletionDt,103) as ActualCompletionDt
											,ProjectCat
											,ProjectDelReason
											,StandardRestruct
										    ,@EffectiveFromTimeKey
