...
declare @p11 int
set @p11=-1
exec [dbo].[CustNPAMOCStageDataInUp] @Authlevel=N'2',@TimeKey=25999,@UserLoginID=N'test_two',@OperationFlag=N'1',@MenuId=N'24747 ',@AuthMode=N'Y',@filepath=N'CustlevelNPAMOCUpload.xlsx',@EffectiveFromTimeKey=25999,@EffectiveToTimeKey=49999,@UniqueUploadID=NU
...
BEGIN
	...
	SET 	@ExcelUploadId=(SELECT MAX(UniqueUploadID) FROM  ExcelUploadHistory)
		...
		SELECT
			...
			,'CUST'
			,H.IssuerEntityId
			--select * from InvestmentIssuerDetail

			--select * from CalypsoCustlevelNPAMOCDetails_stg   where menuid=24747 
		...
		SELECT
			...
			--select * from MetaScreenFieldDetail   where menuid=24747 
			--select * from MetaScreenFieldDetail   where menuid=126
		FROM CalypsoCustlevelNPAMOCDetails_stg A		
	...
	UNION ALL
	Select UCICID, 'SecurityValue' FieldName from CalypsoCustlevelNPAMOCDetails_stg Where isnull(SecurityValue,'')<>''  --SecurityValue Is not NULL
	UNION ALL
	...
	Where A.MenuId=24747  And A.IsVisible='Y'

----	-------------------
----	select * from #CustMocUpload
----	select CtrlName,* from MetaScreenFieldDetail A Where A.MenuId=24747  And A.IsVisible='Y'
----	update MetaScreenFieldDetail 
----	set CtrlName='AdditionalProvision'
----	 Where MenuId=24747  And IsVisible='Y' and entityKey=1406
----	 Asset Class
----NPA Date
----Security Value
...
----select * from MetaScreenFieldDetail_21062021 where menuid=24747 
----select * from MetaScreenFieldDetail where menuid=24747 
	---------------------
