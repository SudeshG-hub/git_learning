...
[MOCProcessed] [varchar] (2) COLLATE SQL_Latin1_General_CP1_CI_AS NULL
) ON [PRIMARY]
GO
ALTER TABLE [PRO].[CustomerCal_Main] ADD CONSTRAINT [CK__CustomerC__Effec__251C81ED] CHECK (([EffectiveToTimeKey]=(49999)))
GO
ALTER TABLE [PRO].[CustomerCal_Main] ADD CONSTRAINT [Customercal_Main_CustomerEntityID] PRIMARY KEY NONCLUSTERED ([CustomerEntityID], [EffectiveFromTimeKey], [EffectiveToTimeKey]) ON [PRIMARY]
GO
CREATE CLUSTERED INDEX [CustomerCal_Main_ClsIdx] ON [PRO].[CustomerCal_Main] ([EntityKey]) ON [PRIMARY]
GO
